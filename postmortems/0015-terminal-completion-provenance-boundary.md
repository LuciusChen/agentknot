...
Post-change Luna/max Job `job_2f15b9f9-8061-4871-a2ab-e2b75d059333` received 184 Pi frames while persisting 67 events and zero `worker.raw` events; its complete Job record occupied 174,496 bytes. This is real-path confirmation of the filter, not a same-task A/B or a fixed reduction claim. The remaining dominant event class was `worker.tool.completed` at 142,475 serialized bytes (81.6% of the record), so bounded tool evidence is the next measured record-growth problem rather than a reason to broaden this filter silently.

The same run also exposed that the valid machine report suffix is removed from terminal `result.output` only after streaming `worker.text.delta` has already been observed; human streaming CLI output can therefore display the marker. That behavior is not misdocumented as result stripping, but it remains a separate presentation/streaming boundary to resolve explicitly rather than by suppressing arbitrary text deltas.

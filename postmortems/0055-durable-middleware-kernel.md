...
Parent recovery requires the same immutable boundary before it can safely dispatch a child that was planned but never admitted. New dispatching parents therefore store the deterministic controller-assessment policy projection and one integrity-checked input as admission state; every worker and reviewer Job now derives from it rather than re-inspecting mutable source. A serial-dispatch regression test mutates the controller workspace after the first child starts and proves both children still receive the admitted dirty input while the controller mutation remains untouched. Parent lease reclaim, orphan adoption, reviewer resumption, and conservative interrupted validation remain open, so scheduler lifetime ownership is still transitional.

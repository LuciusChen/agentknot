...
- The record lives below one stable AgentKnot-owned per-user application-state directory. Transient session variables must not select different rendezvous paths. This corrects the original `XDG_RUNTIME_DIR`/home-fallback choice after the heterogeneous controller failure recorded in [0073](./0073-stable-multi-session-broker-rendezvous.md). It is not stored in a shell profile, target repository, controller transcript, or AgentKnot Job store.

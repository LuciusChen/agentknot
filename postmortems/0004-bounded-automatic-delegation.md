...
### 2026-08-09 — repeated missing acceptance-criteria field

Real dogfood orchestrations `orchestration_faf39e38-5a69-4092-8e31-0a24900c581e` and `orchestration_685ed794-53a8-46af-b3b2-151dbf3e6a00` both produced syntactically valid Luna planner JSON whose only observed contract error was a missing subtask `acceptanceCriteria` field. In both outputs the model had embedded a detailed `Acceptance criteria:` section inside `prompt` instead. Strict validation correctly rejected both plans and persisted the configured upstream fallback without dispatching children, but useful delegation completion was lost.

The first repair is deliberately limited to the planner instruction: every subtask must contain the four separate keys and `acceptanceCriteria` must be a non-empty JSON string array rather than prose embedded only in `prompt`. Runtime validation remains unchanged, no field is inferred or synthesized, and no automatic repair call is added. This avoids weakening the plan contract or spending a second model request before the lower-cost prompt correction has real evidence. Orchestration `orchestration_7852d4f5-1832-4af7-9ec4-0cc85fe7bba0` produced a valid pre-change plan for the slice. After the change, suggest-only orchestration `orchestration_ece33e01-b55c-4362-99fc-10ccff8fca0c` produced a delegated subtask with five separate criteria, while auto orchestration `orchestration_df61c8d4-69db-41a9-b36e-a94f15170c98` returned a valid `do-not-delegate` assessment for a trivial single-test task and correctly kept it upstream. These two samples justify retaining the lower-cost prompt-only correction for now; recurrence should reopen the decision and require explicit persisted repair-attempt provenance before adding another model call.

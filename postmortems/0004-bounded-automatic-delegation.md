...
The first repair is deliberately limited to the planner instruction: every subtask must contain the four separate keys and `acceptanceCriteria` must be a non-empty JSON string array rather than prose embedded only in `prompt`. Runtime validation remains unchanged, no field is inferred or synthesized, and no automatic repair call is added. This avoids weakening the plan contract or spending a second model request before the lower-cost prompt correction has real evidence. Orchestration `orchestration_7852d4f5-1832-4af7-9ec4-0cc85fe7bba0` already produced a valid pre-change plan for this slice; a separate post-change dogfood run is still required to judge whether the prompt hardening is reliable enough or a bounded, persisted repair attempt is justified.

...
- 2026-08-12: A controller attempted to use `context` through an MCP tool definition retained by an already-running Codex session and received `Unrecognized key: "context"`. Source, compiled output, and the fresh-process MCP contract already exposed `context`, and the deterministic stdio test successfully admitted an assessment containing it. The failure was stale per-session tool-schema state after a plugin update, not a missing AgentKnot transport implementation. No duplicate schema or controller-specific fallback was added; the existing installation guidance to start a new controller session remains authoritative.

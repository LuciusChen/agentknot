...
The attempted delegated pair produced correct equivalent answers through Pi / OpenCode Go / DeepSeek V4 Flash / `thinkingLevel=max`, but is invalid as an A/B comparison:
...
The old arm introduced a task-specific `maxToolCalls: 10`, while the compact arm did not; the compact arm's downstream report was also longer and its wait path returned through an extra active controller turn. Tool-count limits are not a valid semantic scope mechanism and were later removed entirely by decision 0083. The pair therefore supplies two successful dogfood records but **no A/B token, latency, or end-to-end efficiency evidence**. Durable records remain in Orchestrations `orchestration_1cf59588-970f-4435-9a29-4f70adb65b04` and `orchestration_6a384069-acc1-4b24-99e6-2c2bf7682c9d`, with Jobs `job_54ab2f40-8fb8-495f-a639-42555e513902` and `job_d91a6137-80c3-4350-981d-af5500a65646`; their usage delta must not be cited as a comparison.
...
- Fresh external exact usage now covers one controlled explicit Skill-load pair. The attempted delegated pair is invalid because its execution-count and controller paths differ. A future end-to-end pair must use equivalent context boundaries with no tool-count field; implicit selection and repeated efficiency remain separate promotion gates.

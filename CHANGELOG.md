...
- Automatic delegation defaults to `maxChildren: 2` and `maxConcurrency: 2` when dispatch limits are omitted; each value is capped at 6 and concurrency cannot exceed the child count. The repository dogfood configuration remains a six-task pool with four active Pi/OpenCode Go/Luna slots after a current four-child orchestration succeeded. Unpaced concurrent direct `Job` calls bypass the delegation semaphore and are not orchestration-capacity evidence; callers own their admission control in v1 ([incident 0018](postmortems/0018-pi-concurrency-startup-ceiling.md)).

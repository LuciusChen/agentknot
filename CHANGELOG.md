...
- Optional vendor-neutral shadow route-selection evidence under `delegation.dispatch.routeSelection`: omission disables selection, `shadow` is the only accepted mode, 1–20 ordered rules validate candidate routes at config load, and first-match/default suggestions are persisted in the plan and child metadata while actual execution remains on `dispatch.defaultRoute`; separate measured scorecards are required before model rankings can drive automatic selection ([decision 0016](postmortems/0016-shadow-route-selection.md)).

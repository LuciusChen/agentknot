...
The optional schemaVersion 1 `TaskAssessment.context` is the controller-authored `ContextManifest`; the compatibility name `TaskContext` refers to the same structure. Its existing `summary`, `relevantPaths`, and `constraints` remain required when the manifest is present. Optional `references` contain exact bounded objects with required `id`, `kind`, opaque `locator`, `source`, and `trust: "unverified"`, plus optional `revision`, `sha256:<lowercase-hex>` `digest`, and `summary`. Reference IDs are unique within the manifest, all strings reject control characters, and the complete serialized manifest remains within the existing 2 KiB limit. Validation returns fresh objects, admission persists and plan-hashes the metadata, and child prompt construction projects it once. No current boundary opens, fetches, searches, verifies, or grants authority through a locator; TypeScript, HTTP, CLI, and MCP accept the same manifest contract.

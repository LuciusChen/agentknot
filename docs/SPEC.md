...
Codex and Claude hooks honor their explicit environment first; otherwise each calls `agentknot client --json` once, passes an available URL as `--server` to every remaining call, preserves the existing repository-local opt-in for `unconfigured`, and fails without local or model fallback for `unavailable` or malformed discovery. They resolve the Git root and check one expected local config path only when unconfigured; they do not scan a repository for AgentKnot internals. This slice adds no daemon manager, service installer, new protocol, or shell mutation. Deterministic verification passes 213 of 213, including the cross-process and Codex/Claude hook-parity gates.

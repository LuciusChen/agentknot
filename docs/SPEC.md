...
Job terminal success is not WorkOrder acceptance. The minimal `CandidateRecord`, `ReviewRecord`, and `DispositionRecord` remain separate domain records rather than interpretations of Job/Orchestration status or advisory-review evidence. Candidate, Review, and Disposition have explicit WorkOrder-rooted CLI compositions. The Review CLI records supplied evidence and never launches a reviewer or infers a verdict; the Disposition CLI records supplied accept/discard input and never infers or applies the decision.
...
`SqliteReviewStore` wraps `SqliteDurableRecordStore` with the `Review` kind in a caller-selected local `agentknot.sqlite` directory and exposes create/read/list/event-cursor operations. The snapshot and one `review.created` event commit atomically; general Review save is rejected. Candidate validation and Review persistence are separate stores and not one cross-store transaction, with no recovery or reconciliation. `agentknot task-review WORK_ORDER_ID --reviewer NAME --summary TEXT [--finding-json JSON] [--candidate CANDIDATE_ID] [--json]` is an explicit controller/presentation composition that records one new already-authored Review through this API; `task-reviews WORK_ORDER_ID [--json]` reads all Reviews linked through the WorkOrder's Candidates after restart. A unique Candidate is selected automatically and an ambiguous set requires explicit selection. These commands add no HTTP workflow, reviewer launching, automatic review, source-record transition, verdict, Disposition, acceptance, discard, artifact application, commit, merge, push, or promotion behavior.
...
`SqliteDispositionStore` wraps `SqliteDurableRecordStore` with the `Disposition` kind and exposes record/create, get, getForCandidate, list, and event-cursor reads. The snapshot and its one event commit through the existing atomic create transaction; general Disposition save is rejected, and explicit unsupported schema versions fail. `agentknot task-disposition WORK_ORDER_ID --decision accept|discard --decided-by NAME --rationale TEXT [--review REVIEW_ID] [--json]` is an explicit controller/presentation composition that selects a unique linked Review or requires an explicit identity, then records supplied input through the domain service; `task-dispositions WORK_ORDER_ID [--json]` reads linked decisions after restart. These commands never infer, apply, promote, commit, merge, push, delete, or otherwise mutate the Candidate artifact or canonical workspace. They add no HTTP workflow, automatic disposition, source-record transition, lifecycle status, recovery, reconciliation, or promotion behavior; Job success remains technical execution evidence rather than acceptance.
...
The controller first constructs one bounded parent task and one strict assessment. The preferred adapter calls `agentknot_broker_status`; if it reports `stopped` or `unavailable` with `launchConfigured: true`, the controller tries `agentknot_broker_start` once before `agentknot_delegation_policy` and `agentknot_orchestration_start`. The lifecycle may remove only an identity-matching crash-stale discovery record; malformed or unidentified ownership still fails. `agentknot_orchestration_follow` performs one bounded durable cursor wait, later `agentknot_orchestration_status` calls remain non-blocking snapshot reads, `agentknot_job_output` reads one explicitly requested output page, and `agentknot_artifact_preview` reads one exact patch. Missing launch configuration or failed startup is reported without repository scanning, a controller-local runtime, or route/model fallback. The Skill's CLI fallback uses the same broker API through `agentknot client --json` and `orchestrate --server ... --handoff-json`. Informational chat, requirements/product decisions, artifact acceptance/promotion, commit, push, merge, and deployment remain upstream.

`--handoff-json` is a CLI-only projection of the authoritative terminal `OrchestrationRecord`, not a second persisted schema. It retains schema version, orchestration identity/status, source/delegation audit values, plan decision/hash/reasoning, assessment classification, compact subtask route evidence, child status, bounded existing completion-report evidence, output availability/retained byte size, compact error, result action/artifact review, optional quality-review and artifact-validation evidence, terminal error, and compact read-only artifact size/SHA-256/base/changed-path verification. It never includes `children[].output` by default. Completion arrays retain at most 20 entries and each projected text field at most 256 UTF-8 bytes; child error messages retain at most 2 KiB. Persisted validation stdout and stderr become at most 2 KiB tails apiece. The complete result has one non-configurable hard budget—default and maximum are both 32 KiB—measured as `Buffer.byteLength(JSON.stringify(handoff), "utf8")`. When needed, deterministic degradation first removes other presentation detail, then completion notes, checks, changed paths, and remaining risks; it may further compact worker-report, artifact-attempt, route, and error detail, but never removes a child or its identity, status, minimum route identity, completion outcome, output availability/retained byte count/persistence-truncation flag, artifact final status, or minimum failure code/message. Top-level `handoffTruncation` records `applied`, `maxBytes`, the pre-degradation `originalBytes`, an `omittedItems` count of removed entries/optional fields (a shortened text field counts once), and sorted affected child IDs. The projection omits request prompt/workspace/metadata, policy and execution snapshots, events, acceptance criteria, execution prompts, and the duplicate result child array. Full CLI, HTTP, and TypeScript record surfaces and durable output remain unchanged.

On demand, `AgentKnotRuntime.readJobOutput`, `AgentKnotHttpClient.readJobOutput`, `GET /v1/jobs/:id/output`, and MCP `agentknot_job_output` expose one identical page of the output already retained in `JobRecord.result.output`. The optional `subtaskId` is an expected delegated identity, `cursor` is a UTF-8 byte offset, and `maxBytes` defaults to 4 KiB with an accepted range of 4 bytes through 16 KiB. Returned pages contain only identity, chunk, cursor/next-cursor, `hasMore`, total retained bytes, and existing persistence-truncation evidence; missing Job, mismatched subtask, and absent output are distinct structured unavailable results. Handoff `outputBytes` and reader `totalBytes` both describe retained UTF-8 bytes, while `outputTruncated`/`outputTruncation` means the 1 MiB durable-retention boundary already discarded original bytes that pagination cannot recover. A cursor must be in range and on a code-point boundary. Because the current JSON record representation reads and parses the retained record before slicing a page, each page is O(n) in retained record/output size. MCP preserves equivalent text JSON and `structuredContent` for compatibility, but each form contains only the bounded page or handoff. This boundary targets upstream context pressure, not total-token reduction.
...
- ReviewRecord creation is explicit through `ReviewService` and the WorkOrder-rooted `task-review` CLI composition. A Review is immutable and status-free, stores one Candidate ID, bounded reviewer/summary text, and bounded structured findings, and has no general save, verdict, recommendation, acceptance/discard value, lifecycle status, or `updatedAt`. Its read-only Candidate validation and Review persistence are separate stores rather than one cross-store transaction; it never converts orchestration quality-review evidence. `task-reviews` is a restart-safe read projection, and neither command launches a Reviewer Job or claims automatic review.
- DispositionRecord recording is explicit through `DispositionService` and the WorkOrder-rooted `task-disposition` CLI composition. A Disposition is immutable and status-free, stores one Candidate ID, one belonging Review ID, `accept` or `discard`, bounded `decidedBy` and rationale, and one purpose-specific event; its deterministic fixed-length Candidate-derived identity permits one record per Candidate, exact replay, and explicit conflict without general save. Candidate/Review validation and Disposition persistence are separate stores rather than one cross-store transaction, and the record never mutates or promotes artifact/workspace state. `task-dispositions` is a restart-safe read projection, and neither command infers a decision or applies an artifact.
...
- ReviewRecord creation/read/list/event-cursor persistence, restart round-trip, schemaVersion 1 status-free shape without verdict/recommendation/acceptance/discard/status/`updatedAt`, bounded invalid-input rejection, low/medium/high finding validation, defensive-copy behavior, missing-Candidate rejection, multiple reviews for one Candidate, source-record nonmutation, rejection of general Review updates, plus WorkOrder-rooted human/JSON CLI presentation, restart reads, explicit ambiguity handling, and preservation of Candidate evidence;
- DispositionRecord accept/discard recording/read/getForCandidate/list/event-cursor persistence, fixed-length Candidate-derived identity, exact replay without a second event, different-input and concurrent conflict, missing/mismatched Candidate/Review rejection, restart round-trip, strict bounds, defensive copies, source/artifact nonmutation, atomic create/event persistence, unsupported schema rejection, general-save rejection, plus WorkOrder-rooted human/JSON CLI presentation, explicit Review ambiguity handling, replay/conflict presentation, restart reads, and preservation of all source records;

...
Codex and Claude hooks honor their explicit environment first; otherwise each calls `agentknot client --json` once, uses an available URL for the policy lookup, and preserves the resolved-repository local-config opt-in for `unconfigured`. Unavailable or malformed discovery produces bounded unavailable controller context without selecting a local runtime, worker, provider, or model fallback. The separate bounded controller-session workspace records do not change endpoint discovery or runtime ownership. Neither path scans a repository for AgentKnot internals.
...
The Skill is an adapter over the existing CLI, not another public payload. The controller first constructs one bounded parent `TASK` and one strict `ASSESSMENT`; the Skill requires `command -v agentknot`, resolves the Git root, and calls `agentknot orchestrate --assessment-json "$ASSESSMENT" --handoff-json --prompt "$TASK"` with the adapter source. It consumes compact terminal/artifact evidence and previews only valid non-empty artifacts. The CLI uses an explicitly selected or registered server when available and must not open local storage in that mode. A missing prerequisite stops the Skill without substituting another command, local runtime, worker, provider, or model. Informational chat, requirements/product decisions, artifact acceptance/promotion, commit, push, merge, and deployment remain upstream.

...
Because `UserPromptSubmit` has no category matcher, every non-explicit prompt in an `auto` repository reaches the configured Luna planner before it may be retained upstream. This is an explicit latency, quota, and prompt-data consequence of `mode: "auto"`; `off` and `suggest` do not pre-dispatch. Hook or CLI failure returns bounded upstream context and never silently substitutes a worker, provider, or model.

Manifest/Skill validation, deterministic controller parity, a populated delegated handoff fixture, and a pre-model hook execution fixture are current evidence. An initial real Codex five-file audit entered orchestration before the first controller-model request, used Luna/max for one medium non-parallel child, and reduced upstream input from 155,851 to 17,951 tokens without controller tool calls. Its smaller output cap was enlarged afterward and covered deterministically but not re-benchmarked. This single workload does not prove universal savings or Claude parity, so the integrations remain experimental ([decisions 0027](../postmortems/0027-controller-native-integration-boundary.md), [0029](../postmortems/0029-controller-cli-and-single-child-delegation.md), and [0030](../postmortems/0030-pre-model-controller-dispatch.md)).

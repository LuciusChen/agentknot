...
Each package contains the same controller-owned handoff Skill and common MCP manifest, with controller-specific explicit invocation metadata only. No `UserPromptSubmit` hook injects a developer message, captures prompts, tracks sessions, discovers repositories, or owns runtime lifecycle. New, continued, resumed, and repository-switched sessions rely on controller-native Skill discovery; the upstream controller supplies the current workspace.

`agentknot --server URL` and `AGENTKNOT_SERVER_URL` select the existing HTTP API as the CLI transport. A client-capable command with no explicit selector uses the registered endpoint according to the discovery contract above. In server mode the CLI never calls `createRuntime()`, loads configuration, acquires file locks, or reconciles snapshots. `run` and `orchestrate` submit and wait for terminal records; catchable termination requests cancellation from the server. They follow `GET /v1/jobs/:id/events?after=SEQUENCE` or the Orchestration equivalent. The server returns after committed event activity, terminal state, or a fixed five-second heartbeat. Active responses contain the committed event delta, `nextSequence`, and compact schema version, ID, status/phase, update time, route/child status, and last activity. Terminal responses contain `nextSequence` and the full exact record once; they do not duplicate its retained event history beside the record. After transport failure the client retries only that durable ID and cursor at most three times and never creates replacement work. No parallel `/wait` HTTP surface is retained. `--progress` renders compact evidence to stderr, suppressing duplicate unchanged snapshots for 15 seconds. Route, Job, Orchestration, delegation, and artifact read surfaces use the same server. `--server` and `--config` are mutually exclusive; `doctor`, `usage`, and live `run --events` are unavailable through this client slice. Ordinary operations retain a fixed timeout and bounded response body, and each heartbeat remains shorter than that operation timeout; total orchestration duration remains governed by route timeouts. An unavailable server is terminal after bounded same-ID reconnect and never triggers local execution or route/model fallback ([decisions 0062](../postmortems/0062-durable-event-subscription.md) and [0065](../postmortems/0065-retire-http-wait-aliases.md)).

No pre-model hook participates in orchestration or discovery. Broker availability is checked through the common MCP tool in the controller turn; prompt text does not traverse a second controller adapter before that decision.
...
An assessment may include one optional controller-authored `context` object. AgentKnot validates and defensively copies it, includes it in the deterministic plan hash, persists it with the admitted request, and projects the same text before every selected child's task-specific prompt. It is explicitly unverified navigation guidance. Workers begin with that working set, verify only acceptance-relevant facts, and finish with available evidence plus an explicit missing/stale-context report instead of broadening scope. Explicit context constraints take precedence over the generic request to run relevant checks. Correctness never depends on a retained Pi session, provider prompt cache, controller transcript, or separate memory service.
...
Codex `agents/openai.yaml` permits implicit Skill invocation for eligible work without a per-prompt user reminder. The controller must still make the semantic decision and author the assessment in its normal model turn; AgentKnot does not synthesize it. The packages add no prompt hook, bundled runtime, daemon, controller branch in the kernel, local semantic classifier, or special `/goal` API. Their MCP entry launches the separately installed controller-neutral client.

Deterministic evidence verifies exact Codex/Claude manifest, MCP, and Skill parity; absence of prompt-hook packages; strict assessment admission; compact handoff; and one long-lived MCP client across broker restart. Historical hook/session experiments remain evidence for their incidents but do not describe the current controller clients ([decisions 0057](../postmortems/0057-independent-broker-and-thin-controller-clients.md) and [0063](../postmortems/0063-remove-per-prompt-controller-obligations.md)).
...
Production stores permit independent readers and enforce per-record CAS, idempotency, cancellation, fenced execution writes, leaf/parent reclaim, and atomic route-pool selection with Job admission. They do not yet provide a durable global capacity queue, distributed consensus, schema migration, or one transaction spanning the separate Job and Orchestration databases. `createRuntime()` therefore still takes the transitional scheduler-lifetime locks before execution/recovery; this is a migration limit, not record authority.
...
For normal `run` jobs only, the Pi adapter appends one concise provider/model-neutral instruction after the supplied prompt. It asks the final assistant message to end with exactly one single-line suffix beginning `AGENTKNOT_WORKER_COMPLETION_REPORT_V1: ` and followed by schemaVersion 1 `WorkerCompletionReport` JSON containing `taskOutcome`, `changedFiles`, checks, risks, and notes; it requires `blocked` whenever the requested task was not completed. The advisory-review prompt reserves that transport-owned suffix as its only permitted content outside role JSON. The adapter parses only the exact final marked line. A valid blocked line is returned through the common adapter result and settled centrally as a non-retryable failed Job; a missing, malformed, or non-terminal line rejects the adapter attempt after clean process settlement and follows ordinary retry policy. Ordinary prose, tool events, stderr, statistics, and valid empty patch artifacts are never substitutes. Doctor and live probe do not use this protocol ([incident/decisions 0044](../postmortems/0044-required-worker-completion-and-canonical-worktree-id.md) and [0069](../postmortems/0069-repeated-shared-context-scope-trials.md)).
...
- Controller packages retain no workspace, session, prompt, transcript, credential, route, model, endpoint, or Job state and install no prompt hook. Workspace selection comes from the upstream controller's task context and is explicit in the admitted request.
...
- Codex/Claude manifest, MCP, and Skill parity; absence of per-prompt hooks and injected context; common explicit stopped-broker activation from a strict launch profile; and unavailable launch without target-repository or model fallback;
...
- configuration-only doctor wording, successful and failed/unsupported live-probe outcomes, timeout/abort cleanup, CLI exit behavior, exact Pi propagation of route thinking level, and built-in Pi normal-run completion-report prompt injection, valid completed/blocked parsing, route-neutral non-retryable blocked settlement with retained report evidence, missing/malformed/unsupported/non-terminal failure, output preservation, strict end anchoring, summary propagation, live-probe exclusion, and a public empty-artifact regression that cannot succeed without the envelope;

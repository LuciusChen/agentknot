...
`opencode-json` is an experimental supported adapter for the pinned OpenCode CLI JSON run surface. Its adapter-owned argv fixes `run --pure --format json`, passes the exact route as `--model provider/model`, passes an optional exact thinking level as `--variant`, supplies both `--dir` and child `cwd`, and never falls back. Strict chunk-safe JSONL accepts typed objects only. One start is normalized from `step_start`, completed text parts become text events/output, terminal `tool_use` becomes completed-tool evidence, unknown objects remain raw, and an `error` rejects. Success is inferred only from at least one valid `step_finish`, clean exit code zero, and no abort; OpenCode does not expose Pi's explicit `agent_settled`, token-level text streaming, tool-start/update, or retry envelopes, so those capabilities remain unavailable rather than emulated.

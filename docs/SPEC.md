...
Job terminal success is not WorkOrder acceptance. The minimal `CandidateRecord`, `ReviewRecord`, and `DispositionRecord` are current only through their explicit TypeScript domain APIs; the sequence `WorkOrder -> Executor Job -> Candidate -> Review -> Disposition` is implemented as separate records and is not an interpretation of existing Job/Orchestration status or advisory-review evidence.

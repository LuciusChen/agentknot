...
The explicit Skill is an adapter over the existing CLI, not another public payload: it first requires `command -v agentknot` to succeed, resolves the workspace Git root, calls `agentknot orchestrate --handoff-json` with `source: "codex"` or `source: "claude"`, consumes the compact terminal handoff and its embedded artifact verification, and uses the existing preview command only for valid non-empty artifacts. If `AGENTKNOT_SERVER_URL` is set, these commands use that shared execution owner and must not discover local configuration or storage. If the executable or selected server is absent, the Skill stops and reports the prerequisite instead of substituting another command, local runtime, worker, provider, or model. Informational chat, requirements/product decisions, artifact acceptance/promotion, commit, push, merge, and deployment remain upstream.
...
Each package includes the same default `hooks/hooks.json` `UserPromptSubmit` handler and dependency-free Node script. After native trust approval, the hook reads the event's exact prompt and current directory, bypasses explicit Skill invocation, and resolves the Git root. With `AGENTKNOT_SERVER_URL`, it reads policy and executes through that server without locating or opening repository configuration; otherwise it retains the existing explicit/local-config behavior and exits outside Git, without default configuration, or unless the effective delegation mode is `auto`. It then synchronously calls the separately installed `agentknot` CLI with `orchestrate --source <controller> --workspace <root> --delegation inherit --handoff-json --prompt <exact prompt>` before the first controller-model request. `AGENTKNOT_SERVER_URL` and `AGENTKNOT_CONFIG` are mutually exclusive in the hook. Plugin code does not classify task semantics, select routes, broaden eligibility with `force`, or duplicate planner/product policy.

`agentknot --server URL` and `AGENTKNOT_SERVER_URL` select the existing HTTP API as the CLI transport. In server mode the CLI never calls `createRuntime()`, loads configuration, acquires file locks, or reconciles snapshots. `run` and `orchestrate` submit and wait for terminal records; catchable termination requests cancellation from the server. Route, Job, Orchestration, delegation, and artifact read surfaces use the same server. `--server` and `--config` are mutually exclusive; `doctor`, `usage`, and live `run --events` are unavailable through this client slice. Every HTTP operation has a fixed timeout and bounded response body, while total orchestration duration remains governed by route timeouts. An unavailable server is terminal for that client request and never triggers local execution or route/model fallback.

One `agentknot serve` process owns both file stores, startup reconciliation, active-request cancellation handles, and the shared process-local orchestration semaphore. Multiple controller sessions are ordinary concurrent HTTP clients and do not multiply `maxConcurrency`. This is a trusted loopback contract with no authentication or TLS; it is not a daemon manager, durable restart-aware queue, distributed lock, remote worker fleet, or multi-tenant service ([incident/decision 0038](../postmortems/0038-shared-local-controller-runtime.md)).

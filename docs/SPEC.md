...
An assessment may include one optional controller-authored `context` object. AgentKnot validates and defensively copies it, includes it in the deterministic plan hash, persists it with the admitted request, and projects the same text before every selected child's task-specific prompt. It is explicitly unverified navigation guidance. Workers begin with that working set, verify only acceptance-relevant facts, selectively read only task-relevant `workspace-file` candidates, and finish with available evidence plus an explicit missing/stale-context report instead of broadening scope. Explicit context constraints take precedence over the generic request to run relevant checks, and an explicit task output format takes precedence over the generic request for files/checks/risks prose. Correctness never depends on a retained Pi session, provider prompt cache, controller transcript, or separate memory service ([experiment/decision 0081](../postmortems/0081-selective-workspace-context-ab.md)).

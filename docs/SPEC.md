...
Codex and Claude hooks honor their explicit environment first; otherwise each calls `agentknot client --json` once, passes an available URL as `--server` to every remaining call, preserves the existing resolved-repository opt-in for `unconfigured`, and fails without local or model fallback for `unavailable` or malformed discovery. They check one expected local config path only when unconfigured; their separate bounded controller-session workspace records do not change endpoint discovery or runtime ownership. Neither path scans a repository for AgentKnot internals. This slice adds no daemon manager, service installer, new protocol, or shell mutation.
...
The explicit Skill is an adapter over the existing CLI, not another public payload: it first requires `command -v agentknot` to succeed, resolves the workspace Git root, calls `agentknot orchestrate --handoff-json` with `source: "codex"` or `source: "claude"`, consumes the compact terminal handoff and its embedded artifact verification, and uses the existing preview command only for valid non-empty artifacts. The CLI uses an explicitly selected or registered server when one is available and must not open local storage in that mode. If the executable or selected server is absent, the Skill stops and reports the prerequisite instead of substituting another command, local runtime, worker, provider, or model. Informational chat, requirements/product decisions, artifact acceptance/promotion, commit, push, merge, and deployment remain upstream.
...
The default controller hooks pass `--progress` and forward its stderr to the host's expandable hook execution record. Their manifest `statusMessage` is deliberately static and describes planning, worker execution, and verification; core and adapter code do not assume a portable dynamic title update or controller resume API. Synchronous blocking remains the default. A future detached controller-resume adapter may consume durable terminal evidence, but it must be opt-in, idempotent, controller-owned, and preserve synchronous behavior for unsupported controllers.

The `OrchestrationRequest.workspace` is the authoritative primary target and the only repository any worker may modify through its isolated copy. Other repositories named in the task are read-only references. If the requested edit target conflicts with that workspace, the planner retains the task upstream with an explicit mismatch instead of reinterpreting either repository. For `repository-analysis`, `buildPlannerPrompt` also requires every delegated subtask to name the primary target, referenced repositories (or none), exact file/component scope, and non-goals. The generated worker execution prompt repeats a route-neutral boundary: at most five decision-relevant findings and 4,000 characters, concise path/line evidence plus impact, no repository inventory, no source restatement, and no silent scope expansion. These are prompt contracts over the existing free-form task kind; no new persisted schema, controller classifier, worker branch, route, provider, model, or effort choice is introduced.

One exact `127.0.0.1` `agentknot serve` process owns both file stores, startup reconciliation, active-request cancellation handles, and the shared process-local orchestration semaphore, and publishes its product-owned per-user record only after listening. Multiple controller sessions are ordinary concurrent HTTP clients and do not multiply `maxConcurrency`. Non-127 binds remain explicit. This is a trusted loopback contract with no authentication or TLS; it is not a daemon manager, service installer, durable restart-aware queue, distributed lock, remote worker fleet, or multi-tenant service ([incident/decision 0038](../postmortems/0038-shared-local-controller-runtime.md) and [decision 0040](../postmortems/0040-product-owned-local-service-discovery.md)).

When the exact submitted text remains upstream, the hook supplies the planner reason and prohibits resubmitting that same context-free text. The planner receives no controller conversation. If the controller model's existing surrounding context supplies one concrete bounded repository task that the text is continuing, the hook permits exactly one normal delegation entry for a newly reconstructed self-contained task before upstream repository execution; AgentKnot applies ordinary eligibility and keep-upstream policy again. Otherwise work proceeds upstream. The adapter never reads transcripts, classifies continuation phrases, or treats a workspace path as task semantics. This conditional post-model bridge is not equivalent to pre-model dispatch and makes no equal token-savings claim ([decision 0050](../postmortems/0050-context-dependent-continuation-handoff.md)). When work dispatches, the hook supplies the compact handoff and reads only integrity-valid non-empty artifacts through `artifact-preview`; it never applies or promotes them. Child outputs share 24,000 characters, patch previews share 32,000 characters, and the complete additional context is capped at 60,000 characters. The host hook timeout is 3,660 seconds so the route's 3,600-second timeout remains responsible for worker cleanup. Codex `agents/openai.yaml` sets `policy.allow_implicit_invocation: false`, preserving explicit `$agentknot-delegate` without a second implicit path. The packages add no bundled CLI, MCP server, daemon, controller branch in `src`, local semantic classifier, or special `/goal` API.

Because `UserPromptSubmit` has no category matcher, every non-explicit prompt in an `auto` repository reaches the configured planner route before it may be retained upstream. This is an explicit latency, quota, and prompt-data consequence of `mode: "auto"`; `off` and `suggest` do not pre-dispatch. Discovery, CLI, or handoff failure emits the host's blocking `UserPromptSubmit` result, so the submitted prompt does not reach the controller model and cannot silently duplicate the work upstream. If `orchestrate --handoff-json` exits nonzero with structured stdout, the adapter extracts only a bounded orchestration ID, terminal status, and error message before blocking; it does not expose children/artifacts, preview a patch, infer the failed lifecycle phase, or substitute a worker, provider, or model. An intentional `plan.willDispatch: false` result still supplies the planner reason and proceeds upstream normally, subject only to the bounded controller-context recovery rule above ([incident/decision 0048](../postmortems/0048-controller-hook-failure-blocking.md)).

Manifest/Skill validation, deterministic controller parity, a populated delegated handoff fixture, and pre-model hook success/failure fixtures are current evidence. The failure matrix feeds malformed handoff JSON plus a nonzero structured terminal orchestration failure to both controller hooks and proves a blocking bounded reason, exact inherited orchestration arguments, no artifact preview, and no route/worker/provider/model fallback argument. Temporary-install tests also remove either controller package and execute the survivor's copied hook and Skill against the same fake CLI contract; this proves package independence without making a real Claude request. Real Codex experiments separately prove a malformed planner with `fallback: "fail"` admits no child and a deterministic 500 ms Pi route timeout settles the failed parent only after SIGTERM/SIGKILL and managed-worktree cleanup. The timeout exposed and corrected an inaccurate pre-dispatch phase claim; failure reporting remains phase-neutral ([incident 0033](../postmortems/0033-controller-timeout-phase-claim.md)). An initial real Codex five-file audit entered orchestration before the first controller-model request, used Luna/max for one medium non-parallel child, and reduced upstream input from 155,851 to 17,951 tokens without controller tool calls. Its smaller output cap was enlarged afterward and covered deterministically but not re-benchmarked. A later non-empty Pi lifecycle fix used the same Codex model and prompt in two clean worktrees: controller-first execution with a manual delegated review reported 2,266,538 input tokens, while pre-model dispatch reported 141,781 and returned an integrity-valid two-file Luna/max artifact before Codex applied and verified it. The 93.7% reduction is not a pure-direct comparison or a universal savings claim. A subsequent Codex pre-model run dispatched two disjoint Luna/max children concurrently and integrated both verified artifacts, proving multi-child mechanics without a new direct baseline or savings claim. Real Claude and broader workload evidence remain open ([incident 0031](../postmortems/0031-bounded-pi-output-drain.md), [experiment 0032](../postmortems/0032-pre-model-multi-child-evidence.md)).
...
`WorkerCompletionReport` is an optional custom-adapter result, not a Job result field. A strict schemaVersion 1 report contains worker-claimed `changedFiles: string[]`, `checksRun` entries with a non-empty `command`, `outcome` of `passed`, `failed`, or `unknown`, and optional string `notes`, plus `remainingRisks: string[]` and `notes: string[]`. At the adapter boundary, `completionReport: undefined` means no report envelope was detected and `completionReport: null` means an envelope was detected but its JSON was malformed or unsupported; a valid value is copied only after strict validation and a 256 KiB compact-JSON ceiling. Custom TypeScript adapters may omit the report, in which case the orchestrator records a stable unavailable branch without failing an otherwise successful Job. Built-in Pi and OpenCode normal runs require a valid completion envelope and reject missing, malformed, unsupported, or non-terminal envelopes before returning a successful adapter result. Failed or cancelled Jobs without a retained normal result use `not-retained`. AgentKnot never derives this report from output prose, normalized worker events, stderr, session statistics, or an empty verified artifact. Live probes and doctor do not use the envelope protocol ([incident/decision 0044](../postmortems/0044-required-worker-completion-and-canonical-worktree-id.md)).
...
Optional `routePools` sit above complete exact routes. A pool name cannot collide with a route; it has exactly `strategy: "least-active"` and 2–20 unique existing route members. `JobRequest.route`, planner, `delegation.dispatch.defaultRoute`, dispatch rules, and quality review may name an exact route or pool; top-level `defaultRoute` and `doctor` remain exact-route-only. Every quality-review pool candidate must resolve to `maxAttempts: 1`. Before atomic Job creation, one `Orchestrator` reserves the least-active member, counts explicit member Jobs, and rotates equal-load ties. This is local execution-owner state, not persisted capacity, distributed coordination, provider health, quota, or cost data.

The Job keeps the logical target in `request.route`, snapshots one concrete immutable `ResolvedRoute` in `route`, and, for a pool, persists `routePoolSelection` with the pool, ordered candidates, strategy, pre-selection counts/cursor, selected exact route/member index, and rotating tie-break. Retries use only that exact route. Pool selection never switches worker, provider, model, thinking level, credentials, timeout, or attempt policy after admission, and a failed Job never falls back. Planner, child, and reviewer Jobs all use this same evidence; parent records link their Job IDs rather than duplicating exact selection ([decisions 0042](../postmortems/0042-complete-route-pool-balancing.md) and [0047](../postmortems/0047-resumable-controller-binding-and-replaceable-role-pools.md)).
...
For normal `run` jobs only, the Pi and OpenCode adapters append one shared concise provider/model-neutral instruction after the supplied prompt. It asks the final assistant message to end with exactly one single-line suffix beginning `AGENTKNOT_WORKER_COMPLETION_REPORT_V1: ` and followed by schemaVersion 1 `WorkerCompletionReport` JSON containing `changedFiles`, `checksRun` entries with `command`, `outcome` (`passed`, `failed`, or `unknown`), and optional `notes`, `remainingRisks`, and `notes`; it states that every value is a worker-reported claim. Structured planner and advisory-review prompts explicitly reserve that transport-owned suffix as their only permitted content outside the role-specific JSON object, so their strict JSON output rules do not contradict the adapter contract. The adapters parse only that exact marked line at the end of accumulated assistant text. A missing, malformed, unsupported, or non-terminal line rejects the attempt even after Pi settlement or an OpenCode `step_finish` and clean process exit. A valid line is strictly validated with the existing report validator, copied into `completionReport`, and removed together with its separating newline from `result.output` while preceding output remains unchanged. Ordinary prose, tool events, stderr, raw events, usage statistics, and valid empty patch artifacts are never used as substitute completion evidence. Configuration-only doctor and live probe do not append or parse this protocol ([incident/decision 0044](../postmortems/0044-required-worker-completion-and-canonical-worktree-id.md)).
...
- Codex/Claude hook parity for cwd Git roots, one explicit absolute or `~/...` repository path from a non-Git cwd, same-session continuation and exit/resume reuse, Git-root revalidation, and unresolved or ambiguous bypass without AgentKnot CLI calls;

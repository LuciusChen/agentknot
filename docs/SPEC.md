...
Each package includes the same default `hooks/hooks.json` `UserPromptSubmit` and `SessionEnd` handlers plus one dependency-free Node script. After native trust approval, the submit hook reads the event's exact prompt, current directory, and optional controller `session_id`, bypasses explicit Skill invocation, and resolves exactly one Git workspace. The event cwd's Git root is authoritative when available. Otherwise the hook examines at most 16 explicit absolute or `~/...` path tokens, considers only existing files/directories that resolve into Git, and proceeds only when all valid candidates converge on one root. It stores that root in a schemaVersion 1, mode-0600 per-session record keyed by a hash of controller source and `session_id`; later continuation prompts may reuse it after revalidating the Git root, and `SessionEnd` removes the exact record. A missing session ID still permits that prompt's explicit path but creates no binding. Unresolved, ambiguous, invalid, or stale input bypasses automatic entry without scanning home/repositories, parsing the controller transcript, or guessing intent. The hook honors explicit `AGENTKNOT_SERVER_URL` or `AGENTKNOT_CONFIG`; with neither, it calls `agentknot client --json` once and passes an available registered URL to policy, orchestration, and preview calls. `unconfigured` retains the resolved repository-local config opt-in, while unavailable or malformed discovery fails without local or model fallback. The hook then synchronously calls the separately installed `agentknot` CLI with `orchestrate --source <controller> --workspace <root> --delegation inherit --handoff-json --prompt <exact prompt>` before the first controller-model request. `AGENTKNOT_SERVER_URL` and `AGENTKNOT_CONFIG` are mutually exclusive in the hook. Plugin code does not classify task semantics, select routes, broaden eligibility with `force`, scan the repository, or duplicate planner/product policy ([decision 0045](../postmortems/0045-controller-session-workspace-binding.md)).
...
In `shadow` mode, `PlannedSubtask.route` remains `dispatch.defaultRoute`; evidence uses `suggestedRoute`. In `active` mode, `PlannedSubtask.route` becomes the matched `selectedRoute`, or the default target when no rule matches. Dispatch passes that configured target to the ordinary child Job API, whose exact resolved snapshot remains execution authority. Child `agentknotDelegation` metadata carries `taskKind`, `parentComplexity`, and the same evidence. The planner has no route-selection field and cannot name routes. Active selection does not add runtime ranking or fallback: retry remains inside the admitted exact route, and route failure is surfaced without switching worker/model/provider. The repository policy selects exact Pi/DeepSeek Flash/max only for `low`; the planner remains exact Pi/Luna/max, while `medium`, `high`, and no-match children use the configured Pi-Luna/native-OpenCode-Luna pool. Artifact handoff remains unchanged.
...
Validation is eligible only when the delegated parent has exactly one planned and actual child, the authoritative child Job succeeded, and artifact verification finds exactly one integrity/base-valid, non-empty Git patch no larger than 32 KiB. Other cases persist `artifactValidation.status: "skipped"` with a stable reason. Before command execution the workspace manager re-inspects the source for the same clean `HEAD`, re-verifies the exact managed artifact, creates a fresh detached worktree at that base, runs `git apply --check --binary`, and applies the recorded patch only to that disposable worktree. It then runs the configured command there and always attempts exact worktree cleanup. The source repository is never a patch target.
...
Optional `routePools` sit above complete exact routes. A pool name cannot collide with a route; it has exactly `strategy: "least-active"` and 2–20 unique existing route members. Top-level `defaultRoute`, planner, doctor, and quality-review configuration remain exact-route-only. `JobRequest.route`, `delegation.dispatch.defaultRoute`, and dispatch-rule targets may name an exact route or pool. Before atomic Job creation, one `Orchestrator` reserves the member with the lowest process-local active count; exact Jobs addressed directly to any member share that count, and equal-load ties rotate from the previous selection. Admission and every settlement/error path release once. This is local execution-owner state, not persisted capacity, distributed coordination, provider health, quota, or cost data.

The Job keeps the logical target in `request.route`, snapshots one concrete immutable `ResolvedRoute` in `route`, and, for a pool, persists `routePoolSelection` with the pool, ordered candidates, strategy, pre-selection counts/cursor, selected exact route/member index, and rotating tie-break. Retries use only that exact route. Pool selection never switches worker, provider, model, thinking level, credentials, timeout, or attempt policy after admission, and a failed Job never falls back to another member. Orchestration plans may persist a configured pool target; the child Job remains the authoritative exact execution evidence. See [decision 0042](../postmortems/0042-complete-route-pool-balancing.md).
...
4. creates a detached managed worktree for each attempt at that same base, naming it from the existing `job_...` identity exactly once;
...
- Codex/Claude hook parity for cwd Git roots, one explicit absolute or `~/...` repository path from a non-Git cwd, same-session continuation, exact `SessionEnd` cleanup, and unresolved or ambiguous bypass without AgentKnot CLI calls;
...
- strict optional artifact-validation configuration, omission and eligibility skips, exact-artifact revalidation, clean-base refusal, disposable application, shell-free argv execution, pass/nonzero/timeout/output-limit/start/cancellation outcomes, bounded combined output including start errors, concurrent reviewer overlap, separate single-slot admission, cleanup/restart evidence, advisory parent behavior, compact handoff, and source-workspace nonmutation;

...
- Added deterministic parity coverage, native manifest/Skill validation, isolated marketplace install smoke tests, a populated delegated CLI handoff fixture, and strict assessment CLI/HTTP coverage. Current evidence verifies that controller packages contain no prompt hook or raw-prompt forwarding path. A parameterized temporary install removes either controller package and executes the survivor independently.

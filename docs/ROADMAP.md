...
- Keep trigger scope honest: informational conversation and product decisions remain upstream. A prompt hook may resolve one Git workspace and policy, then inject a non-blocking handoff obligation; it must not forward the raw prompt, own semantic classification/planning, start orchestration, wait for workers, choose routes/models, or promote artifacts. The controller authors the strict assessment in its normal model turn.
...
- [x] Let one exact `127.0.0.1` `serve` process publish a product-owned per-user runtime record after listen so later CLI and Codex/Claude hook calls discover it without shell-profile edits or repeated endpoint flags. Preserve explicit config/server precedence, report `unconfigured`/`available`/`unavailable` through `agentknot client --json`, keep stale/malformed discovery from selecting local or model fallback, and add no daemon manager, service installer, repository scan, new protocol, or shell mutation. CLI clients fail explicitly; prompt hooks provide bounded unavailable context. Deterministic verification passes 249 of 249, including cross-process, wait/reconnect, and hook-parity gates ([decisions 0040](../postmortems/0040-product-owned-local-service-discovery.md) and [0053](../postmortems/0053-controller-owned-planning-handoff.md)).
- [x] Close the unpublished service-lifecycle gap without making one operating system the architecture. Replace the external POSIX ownership helper with one built-in SQLite lifetime lock; add a narrow native-host contract with Linux systemd-user and macOS launchd adapters around the same foreground `serve` process; keep installation explicit and reversible; require packaged automatic delegation to use an available shared endpoint and report zero downstream work on pre-admission failure. Add no detached-child daemon, second scheduler/protocol, shell-profile mutation, or multi-workspace orchestration graph ([decision 0054](../postmortems/0054-portable-service-lifecycle.md)).
...
- [x] Add one process-local heterogeneous downstream pool above complete routes. `least-active` selection includes explicit member Jobs, rotates equal-load ties, snapshots and persists the exact member before Job creation, releases reservations exactly once, and never switches during retry. Child and quality-review targets may use pools, with every reviewer candidate limited to one attempt. Repository `advanced-workers`, `routine-workers`, and `review-workers` span Pi and native OpenCode candidates; core code fixes no role to a runtime/model ([decisions 0042](../postmortems/0042-complete-route-pool-balancing.md), [0047](../postmortems/0047-resumable-controller-binding-and-replaceable-role-pools.md), and [0053](../postmortems/0053-controller-owned-planning-handoff.md)).
- [x] Close the native OpenCode real-worker promotion gate. Two error/nonzero failures, two active cancellations, two 1,500 ms timeouts, and one non-empty adapter-owned Luna/max artifact Job retained exact route/attempt evidence, valid artifacts, clean working trees, and no PID or managed-worktree residue. Native OpenCode is promoted as a replaceable complete-route pool member without making Pi privileged and without efficiency, capacity, ranking, or fallback claims ([decisions 0043](../postmortems/0043-native-opencode-lifecycle-soak.md) and [0047](../postmortems/0047-resumable-controller-binding-and-replaceable-role-pools.md)).
...
- Added separate Codex and Claude plugin packages with matching `PostToolUse`/`UserPromptSubmit`/`SessionEnd` hooks. In resolved `auto` workspaces the prompt hook now performs only bounded workspace/policy discovery and injects a non-blocking controller handoff obligation. It never sends the raw prompt to AgentKnot or waits for workers. Codex permits implicit Skill invocation; explicit invocation remains available in both controllers. The separately installed CLI remains a checked prerequisite with no bundled wrapper or fallback.
- Context-dependent resumed prompts keep workspace resolution separate from task recovery. Session focus survives resume under source/session/repository identity, but the controller holding the conversation—not hook or middleware—reconstructs the bounded task and strict assessment. Codex/Claude fixtures cover explicit path binding, `SessionEnd` resume, structured focus updates, ignored transcript-shaped data, no raw-prompt orchestration, and bounded unavailable context ([decisions 0050](../postmortems/0050-context-dependent-continuation-handoff.md) and [0053](../postmortems/0053-controller-owned-planning-handoff.md)).
...
- Added no core branch, MCP server, wrapper, daemon, local semantic classifier, learned router, or duplicate request builder. The hook owns bounded workspace/policy I/O only; the controller owns semantic eligibility/planning and the deterministic composer owns policy, routes, limits, and artifact rules.
- Decision [0053](../postmortems/0053-controller-owned-planning-handoff.md) supersedes the pre-model planning ownership in 0030/0048/0050 while retaining the thin-adapter, CLI prerequisite, workspace binding, deterministic routing, and artifact authority established by earlier records.
...
### Portable service-lifecycle slice delivered

- Replaced the external POSIX ownership process with deterministic built-in SQLite lifetime locks in both canonical storage directories. Cross-process contention, duplicate/canonical path rejection, partial-acquisition cleanup, idempotent close, and process-death release are covered without a native add-on or helper process.
- Added `agentknot service install|start|stop|restart|status|uninstall` over one host contract. Linux renders a systemd-user unit and macOS a LaunchAgent; both run the same compiled foreground `serve` entry, preserve native manager errors, and refuse unsupported platforms or unsafe foreign/insecure definitions.
- Installation persists absolute Node/CLI/config values and an absolute-entry non-secret `PATH`, but no arbitrary environment or credential values. Hooks remain read-only clients and never install services, edit shell profiles, or spawn detached children.
- Packaged Codex and Claude automatic entry now requires an available shared endpoint by default. Only explicit `AGENTKNOT_CONFIG` opts into local execution; target-repository configuration is never inferred, and pre-admission failure is disclosed with no Job or orchestration.

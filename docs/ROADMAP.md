...
1. Move child/reviewer capacity and admission accounting from process-local semaphores/counters to durable state before allowing multiple execution hosts. Until then one broker lifetime lock intentionally prevents competing schedulers; more upstream clients do not require more brokers.

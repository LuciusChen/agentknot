...
The minimal CandidateRecord, ReviewRecord, and DispositionRecord slices are implemented as explicit TypeScript APIs only; they do not add Candidate, Review, or Disposition CLI/HTTP workflows, reviewer launching, automatic review or disposition, artifact application, promotion, or Job/WorkOrder/Orchestration status transitions. Candidate/Job/WorkOrder validation and Candidate persistence remain separate stores, Candidate validation and Review persistence remain separate stores, and Candidate/Review validation and Disposition persistence remain separate stores; no cross-store recovery or reconciliation is added. The domain tests and existing durable snapshot/event gate bound these slices. None authorizes automatic apply, commit, merge, push, or promotion.

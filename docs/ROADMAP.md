...
- planner guidance that separates delegation from parallelism, permits one bounded substantive non-parallel child, keeps objectively trivial one-read work upstream when handoff costs more, reserves parallel plans for independently verifiable dependency-free subtasks with non-overlapping expected write scopes, and uses a sliding dispatcher that refills bounded worker slots from the persisted task pool;
...
- Added separate Codex and Claude plugin packages with repository marketplace manifests, one bounded delegation Skill and one static `UserPromptSubmit` context hook each, source-specific audit identity, and the same existing CLI/terminal/artifact workflow. Explicit invocation is `$agentknot-delegate` for Codex and `/agentknot:agentknot-delegate` for Claude; the hook requires the host to consider the Skill description but does not inspect or dispatch prompts. The separately installed `agentknot` executable is an explicit checked prerequisite, with no bundled wrapper or fallback path.
...
- Added no core branch, MCP server, wrapper, daemon, prompt classifier, or duplicate request builder. Three failed fresh Codex implicit-entry probes justified the later static prompt hook; it injects one bounded Skill-selection instruction and owns no orchestration policy. The pre-slice repository had no prior controller plugin/example implementation to retain or delete.
- Recorded the thin-adapter and runtime-prerequisite boundary in [decisions 0027](../postmortems/0027-controller-native-integration-boundary.md) and [0029](../postmortems/0029-controller-cli-and-single-child-delegation.md).

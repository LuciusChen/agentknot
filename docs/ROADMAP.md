...
- [x] Add one explicit WorkOrder-rooted Review CLI composition. `task-review WORK_ORDER_ID` records one already-authored reviewer/summary/findings payload for a unique or explicitly selected Candidate, and `task-reviews WORK_ORDER_ID` reloads linked Reviews after restart with human-default or complete JSON presentation. It launches no Reviewer Job, converts no orchestration advisory review, infers no verdict, and adds no Disposition, acceptance, application, or promotion behavior.
- [x] Add explicit Disposition after review. Acceptance and discard remain distinct from Job technical success: `DispositionRecord` is an immutable, status-free final controller decision linked to one existing Candidate and one belonging Review, with deterministic one-per-Candidate identity, atomic event persistence, exact replay, and explicit conflict semantics; `accept` never applies or promotes the artifact.
- [x] Add one explicit WorkOrder-rooted Disposition CLI composition. `task-disposition WORK_ORDER_ID` records supplied accept/discard input for the unique or explicitly selected linked Review, and `task-dispositions WORK_ORDER_ID` reloads linked decisions after restart with human-default or complete JSON presentation. It infers no decision and adds no artifact application, deletion, promotion, commit, merge, push, or source-record transition.

The minimal CandidateRecord, ReviewRecord, and DispositionRecord slices remain separate explicit domain APIs with narrow WorkOrder-rooted `task-candidate`, `task-review`, `task-reviews`, `task-disposition`, and `task-dispositions` CLI compositions; none has a domain HTTP workflow. No reviewer launching, automatic review or disposition, artifact application, promotion, or Job/WorkOrder/Orchestration status transition is added. Candidate/Job/WorkOrder validation and Candidate persistence remain separate stores, Candidate validation and Review persistence remain separate stores, and Candidate/Review validation and Disposition persistence remain separate stores; no cross-store recovery or reconciliation is added. The domain and focused CLI tests plus existing durable snapshot/event gate bound these slices. None authorizes automatic apply, commit, merge, push, or promotion.
...
- Added a controller-neutral summary-first `orchestrate --handoff-json` projection so controller Skills receive terminal action, route/child evidence, bounded completion evidence, output availability/retained size, compact errors, artifact review, and artifact verification without ingesting Worker output, full events, repeated prompts, policy snapshots, execution prompts, or separate verification output. A deterministic 32 KiB serialized UTF-8 budget degrades optional summary detail in a fixed order, retains every child's minimum terminal evidence, and reports omissions explicitly. Full records remain authoritative and unchanged. An exact retained Job output can be paged explicitly through the 4 KiB-default/16 KiB-maximum UTF-8 byte reader; only valid non-empty plain patch preview remains a separate review step.

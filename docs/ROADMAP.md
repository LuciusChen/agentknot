...
2. Start a fresh Codex session after installing the current package, confirm the advertised MCP input schema includes optional `TaskAssessment.context`, confirm the identical `SessionStart` obligation on `startup`, `resume`, `clear`, and `compact`, and run one real eligible repository task over that boundary. Validate the resulting artifact upstream and compare upstream/downstream token evidence against the existing controller-owned baseline; do not treat `allow_implicit_invocation` metadata alone as automatic-entry evidence. Run Claude parity only after a Claude plan is available ([decision 0074](../postmortems/0074-session-start-controller-entry.md)).

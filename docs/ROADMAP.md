...
- Continue repeated per-adapter real-worker soaks. Pi remains the promoted reference/planner path; native OpenCode is a human-configured pool member and remains experimental until real failure/cancellation/timeout repetitions and cleanup evidence close the gate.
...
- The earlier two same-task Luna/max pairs remain mixed efficiency evidence: native OpenCode won one and regressed token/elapsed use on the other. They justify no savings claim. Decision 0041 instead accepts demonstrated whole-worker portability as the implementation benefit; deterministic adapter lifecycle/artifact gates are complete, while repeated real failure/cancellation/timeout soak remains open.

...
The minimal CandidateRecord, ReviewRecord, and DispositionRecord slices remain separate explicit domain APIs. Candidate and Review have narrow WorkOrder-rooted `task-candidate`, `task-review`, and `task-reviews` CLI compositions; Disposition has no CLI/HTTP workflow. No reviewer launching, automatic review or disposition, artifact application, promotion, or Job/WorkOrder/Orchestration status transition is added. Candidate/Job/WorkOrder validation and Candidate persistence remain separate stores, Candidate validation and Review persistence remain separate stores, and Candidate/Review validation and Disposition persistence remain separate stores; no cross-store recovery or reconciliation is added. The domain and focused CLI tests plus existing durable snapshot/event gate bound these slices. None authorizes automatic apply, commit, merge, push, or promotion.

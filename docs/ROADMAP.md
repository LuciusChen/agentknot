...
- Ship thin, installable Codex and Claude integrations rather than documentation-only examples. Each integration must provide a deterministic explicit task/delegation entry and, where the controller supports it, a bounded native Skill workflow without hidden per-prompt injection.
...
- Keep trigger scope honest: informational conversation and product decisions remain upstream. Controller-native Skills may trigger eligible delegation, but controller packages install no prompt hook, resolve no workspace/policy/service state, forward no raw prompt through a pre-model adapter, own no semantic classification/planning, start no runtime/orchestration implicitly, wait for no worker in a hook, choose no route/model, and promote no artifact. The controller authors the strict assessment and workspace in its normal model turn ([decision 0063](../postmortems/0063-remove-per-prompt-controller-obligations.md)).
- [x] Remove the pre-model middleware planner and cut over every TypeScript/HTTP/CLI orchestration submission to a required controller-authored `TaskAssessment`. Persist accepted-handoff evidence, retain deterministic policy/routing/scheduling, make Codex implicit Skill invocation available, and delete synchronous raw-prompt orchestration, blocking failures, progress forwarding, terminal parsing, preview reinjection, planner configuration, and planner Job state. Historical schemaVersion 1 planning snapshots remain readable only ([decision 0053](../postmortems/0053-controller-owned-planning-handoff.md)).
...
- [ ] Decide the route-neutral terminal semantics of `taskOutcome: blocked`. A completed read-only audit reported its own constraint violation as blocked; the generic error path retried the identical route/snapshot until the controller cancelled it. Do not add model-specific handling or another completion schema merely to suppress retries; first define whether a worker-reported terminal block is always non-retryable or needs explicit retry evidence ([incident evidence 0069](../postmortems/0069-repeated-shared-context-scope-trials.md)).
...
- Added separate optional Codex and Claude client packages with the same `.mcp.json → agentknot mcp` boundary and controller-authored Skill. The superseded stateless `UserPromptSubmit` obligation has been removed, so neither package injects per-prompt context or owns filesystem, Git, network, CLI, policy, runtime, workspace, or session behavior ([decision 0063](../postmortems/0063-remove-per-prompt-controller-obligations.md)).
...
- Added deterministic parity coverage for native manifests, Skills, and MCP entrypoints, a populated delegated CLI handoff fixture, and strict assessment CLI/HTTP coverage. Current evidence verifies that controller packages contain no prompt hook or raw-prompt forwarding path; a real package-driven repository task remains a Stage 2 promotion gate.

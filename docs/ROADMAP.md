...
- [x] Define a structured completion summary: changed files, checks run, remaining risks, and worker-reported notes, without treating worker assertions as verified facts. The additive summary, deterministic built-in-adapter emission slice, and real Pi/OpenCode Go/Luna/max dogfood evidence are complete.
  - [x] Add the additive schemaVersion 1 `JobRecord.completionSummary` with terminal outcome/attempt, terminal-attempt artifact provenance, and stable unavailable reasons; require valid completion envelopes from normal Pi/OpenCode Jobs so clean process exit or an empty valid artifact cannot stand in for task completion ([incident/decision 0044](../postmortems/0044-required-worker-completion-and-canonical-worktree-id.md)).
  - [x] Validate optional custom-adapter reports strictly and require valid reports from normal Pi/OpenCode Jobs, preserving custom absent/malformed and terminal unretained states without inferring from prose, events, stderr, session statistics, or empty artifacts.
  - [x] Append the exact provider/model-neutral report instruction only to normal Pi/OpenCode runs, parse only an end-anchored single-line suffix, strip valid suffixes from output, and fail built-in attempts with missing/malformed/unsupported/non-terminal envelopes.
  - [x] Keep repository OpenCode profiles inside the permission boundary while allowing a denied tool result to reach the next model turn, so a recoverable bad path does not end the upstream loop before its completion envelope.
  - [x] Keep planner and advisory-review JSON instructions compatible with the transport-owned completion suffix instead of asking the model to satisfy mutually exclusive output contracts.
...
- planner guidance that separates delegation from parallelism, permits one bounded substantive non-parallel child, makes bounded allowlisted repository-file deliverables and evidence-producing `repository-analysis` delegation-first regardless of small size, read-only status, or generic handoff/review overhead, keeps only a direct lookup of one explicit fact from one already identified location upstream on cost grounds, reserves parallel plans for independently verifiable dependency-free subtasks with non-overlapping expected write scopes, and uses a sliding dispatcher that refills bounded worker slots from the persisted task pool;
...
- [x] Preserve context-dependent continuation without transcript parsing. If the exact current prompt is retained because it lacks the controller conversation's task semantics, the shared hook may instruct the controller to reconstruct one concrete bounded self-contained task and use the ordinary AgentKnot entry once before doing repository work. The exact text is not resubmitted, paths remain workspace evidence only, normal keep-upstream policy is re-applied, and this post-model bridge makes no pre-model token-savings claim ([decision 0050](../postmortems/0050-context-dependent-continuation-handoff.md)).
- [x] Delegate bounded evidence-producing repository analysis even when read-only and low-complexity. Add only the free-form `repository-analysis` allowlist value and planner-contract guidance; preserve direct single-fact lookups upstream, existing exclusions, human-authored route policy, and all artifact authority. Add no schema field, hook classifier, controller branch, route fallback, ranking, or repair loop ([incident/decision 0051](../postmortems/0051-evidence-producing-repository-analysis.md)).
- [x] Bound delegated repository analysis after real workers expanded narrow comparisons into long inventories. Require explicit execution workspace, references, exact scope, non-goals, at most five decision-relevant findings and 4,000 characters, concise evidence, and no source restatement. Keep this prompt-only and route-neutral with no result schema or model-specific branch ([incident/decision 0052](../postmortems/0052-bounded-analysis-and-observable-waiting.md)).
- The first deployment run repeated the earlier task with the same Pi/Luna/max child route. Worker duration fell 44.2%, tool calls 46.7%, reported total tokens 73.8%, and retained result characters 84.0%; the report contained four findings. The planner route differed and took longer, so total orchestration improved only 24.1% and the result is not a pure end-to-end model/runtime A/B ([evidence 0052](../postmortems/0052-bounded-analysis-and-observable-waiting.md)).
...
- [x] Replace remote 100 ms full-record polling with middleware-owned terminal wakeup plus five-second compact progress heartbeats. Expose connected phase/route/last-activity evidence, distinguish transport disconnection, retry only the same durable ID three times, and preserve synchronous controller blocking. Do not add transcript parsing, automatic resubmission, a second queue, or controller-specific core branches ([incident/decision 0052](../postmortems/0052-bounded-analysis-and-observable-waiting.md)).
...
- [x] Let one exact `127.0.0.1` `serve` process publish a product-owned per-user runtime record after listen so later CLI and Codex/Claude hook calls discover it without shell-profile edits or repeated endpoint flags. Preserve explicit config/server precedence, report `unconfigured`/`available`/`unavailable` through `agentknot client --json`, fail on stale or malformed registration without local or model fallback, keep `doctor` and `usage` local, require explicit selection for non-127 binds, clean up only the registering server's identity, and add no daemon manager, service installer, repository scan, new protocol, or shell mutation. Deterministic verification passes 250 of 250, including cross-process, wait/reconnect, and hook-parity gates ([decision 0040](../postmortems/0040-product-owned-local-service-discovery.md)).
- [x] Resolve automatic controller workspaces through one source-neutral hook contract: an adapter supplies its bounded source namespace and explicit invocation marker; cwd or one explicit repository binds root plus hashed Git common-directory identity to `session_id`. Resumed sessions retain and revalidate both, rejecting a different repository recreated at the same path. No home scan, transcript parser, semantic classifier, core schema, controller-name branch, or model fallback is added ([decisions 0045](../postmortems/0045-controller-session-workspace-binding.md) and [0047](../postmortems/0047-resumable-controller-binding-and-replaceable-role-pools.md)).
...
- [x] Add a second real adapter without changing core Job semantics. Native OpenCode JSON now shares the route-neutral unit contract and has deterministic exact-argv, auth, JSONL, event, error, cancellation, timeout, exact-child cleanup, diagnostic, usage, completion-report, registry, and worktree-artifact coverage. Exact-route Luna/max live inference and one real isolated Job succeeded through the independent OpenCode auth store; portability/lifecycle evidence, not inconsistent efficiency A/B results, justified implementation ([decision 0041](../postmortems/0041-native-opencode-worker-portability.md)).
- [x] Add one process-local heterogeneous downstream pool above complete routes. `least-active` selection includes explicit member Jobs, rotates equal-load ties, snapshots and persists the exact member before Job creation, releases reservations exactly once, and never switches during retry. The initial child-only slice is extended without another scheduler: planner and quality review may also target pools, with every reviewer candidate limited to one attempt. Repository `advanced-workers`, `routine-workers`, and `review-workers` span Pi and native OpenCode candidates; core code fixes no role to a runtime/model ([decisions 0042](../postmortems/0042-complete-route-pool-balancing.md) and [0047](../postmortems/0047-resumable-controller-binding-and-replaceable-role-pools.md)).
- [x] Close the native OpenCode real-worker promotion gate. Two error/nonzero failures, two active cancellations, two 1,500 ms timeouts, and one non-empty adapter-owned Luna/max artifact Job retained exact route/attempt evidence, valid artifacts, clean working trees, and no PID or managed-worktree residue. Native OpenCode is promoted as a replaceable complete-route pool member without making Pi a privileged planner path and without efficiency, capacity, ranking, or fallback claims ([decisions 0043](../postmortems/0043-native-opencode-lifecycle-soak.md) and [0047](../postmortems/0047-resumable-controller-binding-and-replaceable-role-pools.md)).
...
- [x] Add one bounded read-only usage report over persisted evidence: exact downstream adapter-reported token/cost coverage, an explicitly defined cache-read hit rate, and active/shadow route-selection rule/default counts. Pi RPC and OpenCode JSON normalize exact provider evidence into the same route-neutral shape. Controller usage and upstream/downstream proportions remain structurally unavailable until an exact controller boundary supplies comparable data; no controller transcripts are parsed and no live collector or inferred usage is added ([decision 0034](../postmortems/0034-persisted-usage-observability-boundary.md)).
- [x] Keep provider account-quota headroom outside routing and reporting. OpenCode Go exposes plan limits and console usage, while the pinned CLI exposes local historical statistics rather than an exact account-wide remaining/reset contract; shared credentials may also be consumed by other clients and machines. AgentKnot therefore retains exact per-Job provider-reported usage/cost but does not scrape console sessions, subtract local observations from configured budgets, present inferred remaining quota, or route on that inference ([decision 0034](../postmortems/0034-persisted-usage-observability-boundary.md)).

### Controller integration slice delivered

- Added separate Codex and Claude plugin packages with matching `PostToolUse`/`UserPromptSubmit`/`SessionEnd` hooks. In resolved `auto` workspaces, implicit use runs orchestration before the controller model; a non-Git cwd may bind one explicit repository, update its focus from the latest unambiguous structured tool working directory when the session changes projects, and retain/revalidate it for continuation and exit/resume under the same session. The source-neutral script accepts the previous packaged two-argument hook command so older resumable sessions survive the marker-argument upgrade. Codex disables implicit Skill loading. The separately installed CLI remains a checked prerequisite with no bundled wrapper or fallback.
- Context-dependent resumed prompts now keep workspace resolution separate from task recovery. The exact submitted text still reaches the planner first; when it lacks conversation semantics, the hook no longer forbids all later delegation. It conditionally directs the controller, which already owns the conversation, to reconstruct one bounded task and use the normal entry once before upstream repository execution. Codex/Claude fixtures cover explicit path binding, `SessionEnd` resume, exact prompt forwarding, ignored transcript-shaped event data, retained planner reasons, and removal of the former blanket prohibition ([decision 0050](../postmortems/0050-context-dependent-continuation-handoff.md)).
...
- Added deterministic parity coverage, native manifest/Skill validation, isolated marketplace install smoke tests, a populated delegated CLI handoff fixture, and fake-CLI pre-model hook fixtures. They verify exact-prompt `inherit` dispatch, valid non-empty preview embedding, explicit-Skill bypass, no automatic patch application, and identical Codex/Claude blocking of malformed handoff JSON or nonzero structured terminal failures with bounded reasons, no preview, no upstream controller-model execution, and no fallback command or argument. A parameterized temporary-install fixture removes either Codex or Claude and executes the surviving package's manifest/Skill/hook path against the same fake CLI contract ([incident/decision 0048](../postmortems/0048-controller-hook-failure-blocking.md)).
...
- Recorded the thin-adapter, runtime-prerequisite, failed host-model selection, pre-model boundary, phase-neutral failure correction, and controller-session workspace resolution in [decisions 0027](../postmortems/0027-controller-native-integration-boundary.md), [0029](../postmortems/0029-controller-cli-and-single-child-delegation.md), [0030](../postmortems/0030-pre-model-controller-dispatch.md), [incident 0033](../postmortems/0033-controller-timeout-phase-claim.md), and [decision 0045](../postmortems/0045-controller-session-workspace-binding.md).

### Worker portability evidence

- The route-neutral unit kit now runs against Mock, Pi RPC, and OpenCode JSON. Production references to each real protocol remain in its configuration variant, built-in registry, and adapter boundary; a mock-only `createRuntime` test still proves terminal route/worker and `job.started` evidence without selecting either real runtime.
...
- The pinned OpenCode CLI `v1.18.15` executable is retained outside the repository at `/home/lucius/.local/lib/agentknot/workers/opencode/1.18.15/opencode` with installed-binary SHA-256 `c1971d3d4d42abe8e15b2e320ecc1acbdb8377914d4e2cfa47c9bce2316caa7d`; its release archive checksum was already verified. An independent `opencode-go` credential exists in OpenCode's own mode-0600 auth store, but AgentKnot does not read or translate its value. Native `opencode-luna` live inference and a real attempt-one isolated Job both succeeded at `thinkingLevel=max` without Pi or fallback.
- The earlier two same-task Luna/max pairs remain mixed efficiency evidence: native OpenCode won one and regressed token/elapsed use on the other. They justify no savings claim. Decision 0041 accepts demonstrated whole-worker portability as the implementation benefit, and decision 0043 closes deterministic plus repeated real lifecycle/artifact gates without turning those results into an efficiency claim.
...
### Local service-discovery slice delivered

- One exact `127.0.0.1` `serve` process acquires the per-user discovery ownership, publishes the actual listening URL only after listen succeeds, and removes only its own record identity on graceful shutdown; failed runtime/listen startup leaves no new registration.
- Client-capable CLI commands and both controller hooks use the registered endpoint without shell-profile edits, per-session exports, or repeated `--server` flags. `agentknot client --json` reports `unconfigured`, `available`, or `unavailable`; stale or malformed records are terminal and never open local storage or select a local/model fallback.
- Explicit `--config`, `--server`, `AGENTKNOT_SERVER_URL`, and existing local configuration behavior remain available with documented precedence. `serve`, `doctor`, and `usage` stay local, while non-127 binds require explicit server selection.
- The deterministic 213-of-213 verification includes separate-process discovery through one runtime and identical Codex/Claude hook parity for available, unconfigured, stale/unavailable, and malformed discovery. No daemon manager, service installer, new protocol, repository scan, or shell mutation was added.

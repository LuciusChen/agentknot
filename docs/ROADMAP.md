...
- [x] Evolve that same schemaVersion 1 context into a controller-neutral `ContextManifest` with optional bounded `ContextReference` metadata. Preserve old requests, require actual source and explicit unverified trust, allow optional revision/digest/summary, reject duplicates and aggregate oversize, and cover deterministic prompt projection plus MCP admission without resolving locators ([decision 0077](../postmortems/0077-task-context-reference-manifest.md)).
- [x] Add the first concrete read/authorization boundary for quality review: one durable exact AgentKnot Git-patch grant, same-workspace terminal-source and full identity/base revalidation per attempt, a single-use route-neutral reader, content-free durable audit evidence, and a Pi-only explicit tool mapping that requests bytes on invocation over attempt-owned child-process IPC and sanitizes events. Keep `ContextReference` locators metadata-only; no semantic retrieval, vector store, transcript capture, fixed worker session, external memory dependency, arbitrary path, artifact-content temporary file, or network source is added. This establishes the capability but makes no token-savings claim until repeated A/B evidence exists ([decision 0078](../postmortems/0078-exact-artifact-read-grant.md)).
- [ ] Run repeated same-task quality-review A/B trials comparing prompt-embedded patch evidence with the exact read grant. Record total downstream input/output/cache use, latency, tool calls, verdict validity, and completion reliability before claiming efficiency; use the result to decide whether any `ContextReference` kind should gain an explicit grant issuer.

...
- [x] Test automatic candidate-reference handoff against one real AgentKnot repository owner task before promotion. The 28,487-token / 18.137-second `relevantPaths` arm beat both a generic-reference arm (+5.55% tokens, +22.70% latency) and a controller-known role-summary arm (+23.21%, +39.68%); ordinary tools still searched or read multiple candidates and exact JSON-only output failed in every arm. Remove the uncommitted default-contract draft, keep references explicit, and compact the shared Skill plus `SessionStart` obligation as a deterministic fixed-input reduction without claiming exact model-token savings ([experiment/decision 0082](../postmortems/0082-real-repository-selective-context-gate.md)).

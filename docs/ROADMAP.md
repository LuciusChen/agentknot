...
- [x] Resolve automatic controller workspaces when a Codex/Claude session starts above the target repository: prefer the event cwd's Git root, otherwise accept only explicit existing absolute or `~/...` prompt paths that converge on one Git root, bind that root to controller `session_id`, and delete the exact binding on `SessionEnd`. Preserve silent bypass for unresolved/ambiguous input and add no home scan, transcript parser, semantic classifier, core schema, or model fallback ([decision 0045](../postmortems/0045-controller-session-workspace-binding.md)).
...
- [x] Add one process-local heterogeneous downstream pool above complete routes. `least-active` selection includes explicit member Jobs, rotates equal-load ties, snapshots and persists the exact member before Job creation, releases admission/terminal reservations exactly once, and never switches route during retry. Dispatch defaults/rules may target pools while planner, doctor, quality review, and top-level default remain exact. Repository medium/high/default children now target the Pi-Luna/native-OpenCode-Luna pool; low remains exact DeepSeek Flash/max ([decision 0042](../postmortems/0042-complete-route-pool-balancing.md)).
- [x] Close the native OpenCode real-worker promotion gate. Two error/nonzero failures, two active cancellations, two 1,500 ms timeouts, and one non-empty adapter-owned Luna/max artifact Job retained exact route/attempt evidence, valid artifacts, clean working trees, and no PID or managed-worktree residue. Pi remains the reference/planner path; native OpenCode is promoted as a human-configured pool member without efficiency, capacity, quality-ranking, or fallback claims. OpenCode's first-use `.git/opencode` project-ID write is a documented Git common-directory boundary, not silently cleaned by a racing shim ([decision 0043](../postmortems/0043-native-opencode-lifecycle-soak.md)).
...
- Added separate Codex and Claude plugin packages with repository marketplace manifests, one explicit delegation Skill and matching `UserPromptSubmit`/`SessionEnd` hooks, source-specific audit identity, and the same existing CLI/terminal/artifact workflow. In resolved `auto` workspaces, implicit use now runs `orchestrate --delegation inherit --handoff-json` before the first controller-model request; explicit Skill prompts and non-auto or unresolved workspaces bypass it. A non-Git event cwd may resolve one explicit existing absolute or `~/...` path and retain its Git root for continuation prompts under the same session. Codex disables implicit Skill loading. The separately installed `agentknot` executable remains an explicit checked prerequisite, with no bundled wrapper or fallback path.

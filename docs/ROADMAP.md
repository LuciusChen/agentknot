...
- Added deterministic parity coverage, native manifest/Skill validation, isolated marketplace install smoke tests, a populated delegated CLI handoff fixture, and a fake-CLI pre-model hook fixture. The latter verifies exact-prompt `inherit` dispatch, valid non-empty preview embedding, explicit-Skill bypass, and no automatic patch application. A parameterized temporary-install fixture removes either Codex or Claude and executes the surviving package's manifest/Skill/hook path against the same fake CLI contract.

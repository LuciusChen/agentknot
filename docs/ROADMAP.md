...
- [ ] Add a read/authorization boundary only after a concrete controller-neutral use case defines locator schemes, workspace/snapshot isolation, budgets, audit events, stale-reference behavior, and measurable quality/token gates. Do not add semantic retrieval, a vector store, transcript capture, fixed worker sessions, or an external memory dependency as a shortcut.

...
The minimal `CandidateRecord` is a separate immutable, status-free evidence record. Explicit `CandidateService` creation links one WorkOrder to its bound Executor Job and one exact recorded `git-patch` identity (`path`, `sha256`, and `baseCommit`). It reads the WorkOrder and Job projections before writing the Candidate projection; those read-only validations and Candidate persistence use separate stores and are not one cross-store transaction. `SqliteCandidateStore` reuses the durable SQLite projection and append-only event infrastructure, atomically writing the Candidate snapshot and `candidate.created` event, while Candidate creation retains identity only and never reads, hashes, copies, or applies patch bytes. `agentknot task-candidate WORK_ORDER_ID [--json]` is an explicit controller/presentation composition that requires a successful bound Job and current exact verification of its terminal-attempt artifact before the first write, then reloads a matching Candidate on later sequential calls. It does not mutate WorkOrder/Job records or statuses or imply acceptance, Review, Disposition, promotion, or application; concurrent first calls are not an exactly-once boundary.

### Review evidence boundary

The minimal `ReviewRecord` is a separate immutable, schemaVersion 1, status-free evidence record linked only to one existing CandidateRecord. Explicit TypeScript `ReviewService.create` performs a read-only Candidate existence check, then `SqliteReviewStore` atomically writes one Review snapshot and one `review.created` event through the existing SQLite projection/event infrastructure; those validation and persistence stores are separate and are not one cross-store transaction, with no recovery or reconciliation added. The record retains a bounded non-empty `reviewer` identity, bounded non-empty summary, and defensively copied findings whose entries independently carry `low`, `medium`, or `high` severity, a bounded message, and bounded evidence. It has no verdict, recommendation, acceptance/discard value, lifecycle status, or `updatedAt`; multiple independent Reviews may reference one Candidate.

Domain Review is deliberately not the existing orchestration advisory quality-review result: no verdict or findings are imported, aliased, reinterpreted, migrated, or automatically converted. `agentknot task-review WORK_ORDER_ID --reviewer NAME --summary TEXT [--finding-json JSON] [--candidate CANDIDATE_ID] [--json]` explicitly records already-authored review evidence for the WorkOrder's Candidate, and `task-reviews WORK_ORDER_ID [--json]` reloads all linked Reviews after restart. A unique Candidate is selected automatically; multiple Candidates require an explicit identity rather than an inferred choice. This presentation/controller composition has no HTTP, reviewer-launching, automatic-review, WorkOrder/Job/Orchestration transition, verdict, Disposition, acceptance, discard, artifact application, commit, merge, push, or promotion behavior.

### Disposition decision boundary

The minimal `DispositionRecord` is a separate immutable, schemaVersion 1, status-free controller decision linked to exactly one existing CandidateRecord and one existing ReviewRecord belonging to that Candidate. `DispositionService.record` validates those source projections read-only, then `SqliteDispositionStore` atomically records one `accept` or `discard` decision, bounded non-empty `decidedBy` and rationale, `createdAt`, and one `disposition.recorded` event in a separate store; the fixed domain bounds are 256 UTF-8 bytes for `decidedBy`, 2 KiB for rationale, and 16 MiB for the durable projection. A fixed-length SHA-256 Candidate-derived identity permits exactly one record per Candidate: the first decision succeeds, an exact semantic replay returns the existing record without another event, and any different review, decision, decider, or rationale conflicts explicitly, including races between independent stores.

Disposition is the controller's final recorded decision for the Candidate evidence; `accept` records that decision but never applies, promotes, commits, merges, pushes, deletes, or otherwise mutates the Candidate artifact or canonical workspace. It does not add accepted, discarded, or reviewing status values to WorkOrder, Candidate, Review, Job, or Orchestration, and Job success remains technical execution evidence rather than acceptance. Candidate/Review validation and Disposition persistence are separate stores and are not one cross-store transaction. `agentknot task-disposition WORK_ORDER_ID --decision accept|discard --decided-by NAME --rationale TEXT [--review REVIEW_ID] [--json]` explicitly records or replays supplied controller input for the unique or explicitly selected linked Review, and `task-dispositions WORK_ORDER_ID [--json]` reloads linked decisions after restart. This presentation adds no HTTP workflow, inferred or automatic disposition, artifact integration, recovery, reconciliation, or promotion behavior.
...
Terminal controller handoff is summary-first. It carries bounded completion and artifact evidence plus the availability and retained byte size of Worker output, but does not copy `children[].output` into the default controller response. Its serialized UTF-8 form has a deterministic 32 KiB hard limit: optional details degrade in a fixed priority order, every child remains represented, and `handoffTruncation` makes omissions explicit. Existing durable records retain output under their established record budget; handoff `outputBytes` describes those retained bytes, not the Worker's original unbounded output. A controller that needs more than the summary explicitly reads one UTF-8 byte-cursor page through the bounded Job output surface, which cannot recover bytes already discarded by durable retention. This boundary is intended to reduce upstream context pressure without claiming lower total token use.

The built-in Pi adapter may repair the transport evidence without repeating task work. If one normal assistant turn settles without a valid required completion envelope and without an assistant error, the adapter issues exactly one protocol-only prompt through the same live process/session. A valid response completes the same Job attempt; another invalid settlement becomes a non-retryable settled-worker error. The bounded lifecycle evidence identifies completion-envelope recovery but contains neither its prompt nor raw worker/provider text. This is adapter-owned protocol recovery, not controller planning, task retry, model fallback, or a second completion schema ([incident/decision 0085](../postmortems/0085-in-session-completion-envelope-recovery.md)).
...
- a durable WorkOrder domain root with immutable issued commands, transactional snapshot/event persistence, restart-safe reads, and explicit CAS binding to one already admitted executor Job identity without making WorkOrder launch or control that Job; the minimal `agentknot task` controller flow composes those existing operations with a human terminal report, while explicit JSON mode retains complete machine evidence and `task-show` reloads it by WorkOrder ID after restart;
- a minimal immutable, status-free `CandidateRecord` domain record with explicit service and WorkOrder-rooted `task-candidate` CLI creation, exact WorkOrder/Executor Job binding, and exact recorded Git-patch identity (`path`, `sha256`, `baseCommit`); it reuses the existing SQLite projection/event store to persist the Candidate snapshot and `candidate.created` event, performs read-only source and artifact validation separately from Candidate persistence, retains no patch bytes, changes no source record or status, and adds no Review, Disposition, acceptance, promotion, or application behavior;
- a minimal immutable, schemaVersion 1, status-free `ReviewRecord` linked only to one existing CandidateRecord; explicit TypeScript creation and the WorkOrder-rooted `task-review` CLI validate Candidate existence read-only, then atomically persist one Review snapshot and `review.created` event through `SqliteReviewStore` in a separate store, while `task-reviews` reloads linked Reviews after restart. Bounded reviewer/summary/findings input is defensively copied, multiple reviews per Candidate remain explicit, and the slice adds no reviewer launch, verdict, Disposition, acceptance, discard, application, promotion, or orchestration-quality-review conversion;
- a minimal immutable, schemaVersion 1, status-free `DispositionRecord` linked to one existing CandidateRecord and one ReviewRecord belonging to that Candidate; explicit domain and WorkOrder-rooted CLI recording validate both references read-only, atomically persist one deterministic Candidate-derived record and `disposition.recorded` event through `SqliteDispositionStore`, permit only one disposition per Candidate with exact replay/conflict semantics, and record accept/discard without inferring, applying, or promoting it; `task-dispositions` reloads this evidence after restart;
...
- immediate execution with cooperative timeouts, retries, and cancellation, including persisted bounded exponential backoff with per-Job jitter for structured pre-settlement worker failures, plus bounded exact-child and output-drain supervision in the bundled Pi adapter even when an external event sink does not settle;
...
- a normal-run Pi adapter boundary that limits aggregate text from one tool result to 8 KiB before the next model call, retains explicit truncation evidence, preserves UTF-8, and exempts the exact artifact-read capability; this is a fixed context-safety bound rather than tool-count scope, provider routing, redaction, or a universal token-efficiency claim ([incident/decision 0088](../postmortems/0088-bound-pi-tool-result-context.md));
...
- a read-only CLI and TypeScript usage projection over persisted evidence, with one fixed-shape downstream token/cost or unavailable observation per completed worker attempt, legacy terminal-result fallback, explicit cache-read and route-selection hit formulas, partial/unavailable coverage, and no fabricated controller usage or upstream/downstream proportion;
- optional route-neutral advisory quality review for one successful child and one bounded valid patch, selected by configured parent complexities and a configured single-attempt route or all-single-attempt pool, with an exact single-use artifact-read grant, strict verdict/findings, explicit skipped/unavailable states, no repair or promotion, no embedded domain Disposition, and no measured efficiency claim; this orchestration evidence remains separate from the domain ReviewRecord and DispositionRecord;
...
- additive schemaVersion 1 terminal Job completion summaries with terminal outcome/attempt, controller-captured artifact path provenance, and explicit unavailable states; custom adapters may supply a strict worker completion report, while normal Pi Jobs require one and make one same-session protocol-only recovery after an otherwise successful settlement lacks a valid envelope; a second invalid settlement fails non-retryably so process settlement and an empty valid artifact cannot masquerade as task completion or trigger full task replay; `taskOutcome` applies to the assigned worker-role task rather than an inspected subject's acceptability, and a valid worker-reported block is one terminal non-retryable failure across every route and remains available as worker-reported evidence; a delegated `repository-analysis` that produces any changed path likewise fails once as a read-only-contract violation while retaining the artifact for controller inspection;
...
## Current and planned command-driven domain lifecycle
...
WorkOrder issue/persistence, explicit binding to an already admitted executor Job identity, minimal explicit `CandidateRecord` creation, minimal explicit `ReviewRecord` creation, and minimal explicit `DispositionRecord` recording are implemented in this domain sequence. The developer `task` command is a presentation/controller composition that admits one Job, binds it, and reports its existing result/artifact evidence; it creates no Candidate, Review, or Disposition. The separate `task-candidate` command explicitly verifies and records or reloads Candidate evidence for that WorkOrder without launching work or changing lifecycle state. `task-review` explicitly records already-authored reviewer/summary/findings evidence for one linked Candidate, and `task-reviews` reloads that evidence; neither launches a reviewer nor infers a verdict. `task-disposition` records supplied accept/discard input for one linked Review, and `task-dispositions` reloads it; neither infers a decision nor mutates an artifact. CandidateRecord, ReviewRecord, and DispositionRecord are immutable, status-free records: each source validation and its own persistence are separate stores, neither changes source records or statuses, and DispositionRecord links one Candidate to one belonging Review. Domain Review remains distinct from existing orchestration advisory-review evidence and has no verdict or acceptance authority; Disposition records the controller's final accept/discard decision for Candidate evidence without applying or promoting its artifact. No Job/WorkOrder/Orchestration lifecycle transition treats technical success as acceptance, and no domain record adds accepted, discarded, or reviewing status values.
...
- explicit CandidateRecord creation links exactly one WorkOrder and its bound Executor Job to a recorded Git-patch identity, persists an immutable status-free Candidate snapshot plus event through the existing SQLite/event infrastructure, leaves WorkOrder/Job records and statuses unchanged, and documents the separate-store validation/persistence limitation;
- explicit ReviewRecord creation through the domain service or WorkOrder-rooted `task-review` CLI validates one existing Candidate read-only, atomically persists one immutable status-free Review snapshot plus `review.created` through the existing SQLite/event infrastructure, defensively copies bounded findings, leaves Candidate/WorkOrder/Job/Orchestration records and statuses unchanged, permits multiple reviews per Candidate, and keeps reviewer launch, verdict/disposition/acceptance/discard/application/promotion separate; `task-reviews` is restart-safe presentation over those persisted records;
- explicit DispositionRecord recording through the domain service or WorkOrder-rooted `task-disposition` CLI validates one existing Candidate and one belonging Review read-only, atomically persists one immutable status-free snapshot plus `disposition.recorded` through the existing SQLite/event infrastructure, permits exactly one deterministic Candidate-derived identity, makes exact replay idempotent and different input a conflict, and leaves the Candidate artifact and canonical workspace unchanged; `task-dispositions` reloads the persisted decision, and accept is supplied controller input rather than inferred acceptance, automatic application, or promotion;
...
- completion summaries distinguish controller-captured artifact paths from worker-reported claims, never infer reports from prose/events/stderr/session statistics or empty artifacts, and preserve explicit unavailable reasons; custom adapters may omit reports, while normal Pi Jobs require the exact end-marked envelope, attempt one same-session protocol recovery after an invalid first settlement, and fail the current attempt non-retryably when the recovery settlement is still absent, malformed, unsupported, or non-terminal;
- usage reporting counts each persisted worker-attempt observation once across success, failure, and retry, never double-counts terminal metadata when attempt evidence exists, preserves provider-reported totals and cost without currency normalization, classifies route hits only from terminal plan/policy evidence, and reports controller usage and cross-boundary proportions unavailable until comparable exact controller data is persisted;
...
- artifact validation is disabled by omission, admits only one successful child with one integrity/base-valid non-empty patch, executes one bounded shell-free configured command in a disposable worktree, persists command and cleanup evidence before the terminal parent event, and never mutates the source or promotes the artifact;

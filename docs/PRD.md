...
Domain Review is deliberately not the existing orchestration advisory quality-review result: no verdict or findings are imported, aliased, reinterpreted, migrated, or automatically converted. Review creation has no CLI, HTTP, reviewer-launching, automatic-review, WorkOrder/Job/Orchestration transition, Disposition, acceptance, discard, artifact application, commit, merge, push, or promotion behavior.
...
- a minimal immutable, schemaVersion 1, status-free `ReviewRecord` linked only to one existing CandidateRecord; explicit TypeScript creation validates Candidate existence read-only, then atomically persists one Review snapshot and `review.created` event through `SqliteReviewStore` in a separate store, defensively copies bounded reviewer/summary/findings input, permits multiple reviews per Candidate, and adds no verdict, Disposition, acceptance, discard, application, promotion, or orchestration-quality-review conversion;
...
- explicit ReviewRecord creation validates one existing Candidate read-only, atomically persists one immutable status-free Review snapshot plus `review.created` through the existing SQLite/event infrastructure, defensively copies bounded findings, leaves Candidate/WorkOrder/Job/Orchestration records and statuses unchanged, permits multiple reviews per Candidate, and keeps verdict/disposition/acceptance/discard/application/promotion separate;

...
WorkOrder issue/persistence, explicit binding to an already admitted executor Job identity, and the minimal explicit `CandidateRecord` creation are implemented in this domain sequence. The developer `task` command is a presentation/controller composition that admits one Job, binds it, and reports its existing result/artifact evidence; it is not another domain record or transition and does not create a Candidate. CandidateRecord is immutable, status-free evidence only: its read-only WorkOrder/Job validation is separate from Candidate persistence, and it changes neither source record nor status. Domain Review and Disposition do not yet exist; existing orchestration advisory-review evidence is execution evidence, not the future domain Review. No current transition treats Job success, a Candidate, an artifact, an orchestration advisory verdict, or validation evidence as WorkOrder acceptance. Candidate remains distinct from acceptance, artifact application, and promotion; later slices must introduce Review and Disposition explicitly rather than adding acceptance states to `JobStatus` or silently reinterpreting Orchestration.

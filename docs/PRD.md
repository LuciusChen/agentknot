...
- a minimal immutable, status-free `CandidateRecord` domain record with explicit TypeScript creation, exact WorkOrder/Executor Job binding, and exact recorded Git-patch identity (`path`, `sha256`, `baseCommit`); it reuses the existing SQLite projection/event store to persist the Candidate snapshot and `candidate.created` event, performs read-only source validation separately from Candidate persistence, retains no patch bytes, changes no source record or status, and adds no CLI, Review, Disposition, acceptance, promotion, or application behavior;
...
WorkOrder issue/persistence, explicit binding to an already admitted executor Job identity, minimal explicit `CandidateRecord` creation, minimal explicit `ReviewRecord` creation, and minimal explicit `DispositionRecord` recording are implemented in this domain sequence. The developer `task` command is a presentation/controller composition that admits one Job, binds it, and reports its existing result/artifact evidence; it is not another domain record or transition and does not create a Candidate, Review, or Disposition. CandidateRecord, ReviewRecord, and DispositionRecord are immutable, status-free records: each source validation and its own persistence are separate stores, neither changes source records or statuses, and DispositionRecord links one Candidate to one belonging Review. Domain Review remains distinct from existing orchestration advisory-review evidence and has no verdict or acceptance authority; Disposition records the controller's final accept/discard decision for Candidate evidence without applying or promoting its artifact. No Job/WorkOrder/Orchestration lifecycle transition treats technical success as acceptance, and no domain record adds accepted, discarded, or reviewing status values.

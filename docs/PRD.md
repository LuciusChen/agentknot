...
- fenced leaf startup recovery with admitted-input integrity, cancellation precedence, explicit lost-attempt evidence, and next-attempt-only retry; parent orchestration reconciliation remains fail-without-resume during the next migration slice;
...
The current production stores provide transactional projections, append-only event cursors, CAS revisions, idempotent admission, durable cancellation intent, and fenced leases. Leaf git-worktree admission retains a bounded reference to an exact-ID integrity-checked input patch. After the prior lease expires, an execution-owning runtime may reclaim a queued leaf from that immutable input or record a running attempt as lost before consuming only its next configured retry; durable cancellation wins before replay and the exact persisted route remains fixed. Parent Orchestrations still use fail-without-resume reconciliation, and read-oriented runtimes perform no recovery. The system does not yet provide a restartable capacity queue, multi-host scheduler, distributed coordination, or automatic cleanup of arbitrary descendants and worktrees left by an uncatchable hard process crash.
...
The current scheduler is still single-owner while Stage 3 parent recovery is in progress. Execution-owning construction first claims both canonical storage directories; read-only controllers can inspect the transactional store concurrently but cannot execute or recover through that runtime. Admitted executions additionally hold renewable fenced record leases and persist cancellation intent, so stale storage writes, cross-session cancellation, and leaf reclaim no longer depend on the HTTP process map. The lifetime scheduler lock is not a distributed lock or hostile-process sandbox and will be removed only after parent recovery and durable capacity accounting pass.
...
- Hard `SIGKILL`, host loss, or another uncatchable failure bypasses shutdown handlers; fenced leaf recovery repairs supported persisted Job boundaries after lease expiry, while parent recovery cannot yet resume and no path universally proves ownership of leftover processes or worktrees.

...
- a minimal immutable, schemaVersion 1, status-free `DispositionRecord` linked to one existing CandidateRecord and one ReviewRecord belonging to that Candidate; explicit TypeScript recording validates both references read-only, atomically persists one deterministic Candidate-derived record and `disposition.recorded` event through `SqliteDispositionStore`, permits only one disposition per Candidate with exact replay/conflict semantics, and records accept/discard without applying or promoting the artifact;
...
- explicit DispositionRecord recording validates one existing Candidate and one belonging Review read-only, atomically persists one immutable status-free snapshot plus `disposition.recorded` through the existing SQLite/event infrastructure, permits exactly one deterministic Candidate-derived identity, makes exact replay idempotent and different input a conflict, and leaves the Candidate artifact and canonical workspace unchanged; accept is a recorded controller decision, not automatic application or promotion;

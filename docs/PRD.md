...
WorkOrder issue/persistence, explicit binding to an already admitted executor Job identity, minimal explicit `CandidateRecord` creation, and minimal explicit `ReviewRecord` creation are implemented in this domain sequence. The developer `task` command is a presentation/controller composition that admits one Job, binds it, and reports its existing result/artifact evidence; it is not another domain record or transition and does not create a Candidate or Review. CandidateRecord and ReviewRecord are immutable, status-free evidence only: each source validation and its own persistence are separate stores, neither changes source records or statuses, and ReviewRecord links only to an existing CandidateRecord. Domain Review is distinct from existing orchestration advisory-review evidence and has no verdict, recommendation, acceptance/discard value, or lifecycle status; Disposition does not yet exist. No current transition treats Job success, a Candidate, an artifact, an orchestration advisory verdict, a domain Review, or validation evidence as WorkOrder acceptance. Candidate and Review remain distinct from acceptance, discard, artifact application, and promotion; later slices must introduce Disposition and any such controller decision explicitly rather than adding acceptance states to `JobStatus` or silently reinterpreting Orchestration.

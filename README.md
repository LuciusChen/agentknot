...
For a real configured repository, install the CLI once, start the middleware in an ordinary foreground terminal, and submit an orchestration from any controller session:
...
No shell-profile edit, systemd unit, launchd agent, Unix socket, or controller-specific daemon is required. The existing optional `agentknot service` host adapters remain available during the Stage 3 migration but are not a correctness prerequisite and are no longer the recommended setup. To make new Codex or Claude sessions enter this flow automatically, continue to [Codex and Claude controller integrations](#codex-and-claude-controller-integrations). Jobs and orchestration records use the configured storage paths; the repository defaults are `.agentknot/jobs/` and `.agentknot/orchestrations/`.
...
| **Current** | Transactional local Job/Orchestration persistence with append-only event cursors, CAS revisions, idempotent admission, fenced execution leases, and durable cancellation intent. | Production `createRuntime()` imports identity-matching legacy JSON evidence without rewriting it and writes one SQLite authority per configured record directory. Record, optional idempotency identity, and first lease admit atomically; independent instances reject stale writes, retain monotonic fences after release, reuse one canonical request identity, observe cancellation by durable ID, and prevent accepted cancellation from becoming success. HTTP wait no longer depends on the admitting server's Promise map. Restart execution recovery is still in progress, so lifetime scheduler ownership and fail-without-resume reconciliation have not yet been removed ([decision 0055](postmortems/0055-durable-middleware-kernel.md)). |
| **Experimental** | Thin installable Codex and Claude controller plugins with explicit or implicit controller-authored handoff. | The prompt hook performs only bounded workspace/policy discovery and injects a non-blocking handoff obligation. The controller owns planning and submits a strict assessment through the normal Skill/CLI; the hook never sends the raw prompt to AgentKnot or waits for workers. Deterministic tests cover package independence, resume binding, dirty-workspace isolation, and bounded unavailable context. The separately installed `agentknot` CLI is required, and real Claude parity remains the promotion gate ([decision 0053](postmortems/0053-controller-owned-planning-handoff.md)). |
...
| **Current** | Product-owned local service discovery for one exact `127.0.0.1` `serve` process. | After listen, the server publishes one per-user record that later CLI commands and Codex/Claude hooks discover; `agentknot client --json` reports `unconfigured`, `available`, or `unavailable`. Cross-process, wait/reconnect, hook-parity, portable ownership, and service-host gates are deterministic; stale or malformed records do not trigger local or model fallback. Discovery is a client convenience, not durable execution authority. |
| **Current** | Optional portable user-service host adapters (transitional). | `agentknot service` can render a mode-0600 systemd-user unit or LaunchAgent for operators who explicitly want native supervision. Hooks never install it or edit shell profiles. Stage 3 removes any requirement for native-service setup and will delete superseded lifecycle code after non-invasive hosting/recovery parity ([decisions 0054](postmortems/0054-portable-service-lifecycle.md) and [0055](postmortems/0055-durable-middleware-kernel.md)). |
...
| **Proposed** | Restart execution recovery/backpressure, authenticated local automation, signed callbacks, approval/policy controls, and an OS-sandbox backend. | Transactional records, events, idempotency, leases, persistent cancellation, and same-ID wait are implemented foundations; reclaim/resume and the remaining capabilities are unavailable until their Stage 3 gates pass. |
...
Controllers choose whether a request enters the leaf Job API or orchestration API. In a workspace whose resolved policy is `mode: "auto"`, the plugin hook only resolves the workspace and policy, then supplies a bounded obligation to the controller model. The controller uses its conversation context to decide eligibility and author the strict assessment; AgentKnot validates that untrusted handoff and applies keep-upstream, route, concurrency, depth, and artifact rules. The resolved Git workspace remains the only writable logical target. Other integrations use the same assessment-bearing CLI, `POST /v1/orchestrations`, or `runtime.orchestrate()` boundary ([decision 0053](postmortems/0053-controller-owned-planning-handoff.md)).
...
For concurrent controller sessions, start the authoritative middleware normally on the exact loopback host:

```bash
# keep this foreground process in a terminal, container, or application supervisor
agentknot serve --config /path/to/agentknot.config.json --host 127.0.0.1 --port 7391
# from another terminal/session
agentknot client --json
```

This does not edit a shell profile or install an operating-system service. Operators who explicitly prefer native supervision may still use the transitional `agentknot service` adapter on supported hosts, but controller plugins never install or require it. Keep credentials in the selected worker's own protected auth store or deliberately configure them in the chosen process host.
...
Read-oriented CLI commands, including `show`, lists, artifact inspection, route and delegation inspection, and both doctor modes, open persisted records without scheduler ownership or startup reconciliation. A TypeScript runtime created with `reconcileOnStartup: false` has the same read-only capability boundary: its execution and reconciliation methods refuse calls. Local `run`, local `orchestrate`, and a valid `serve` invocation still acquire transitional scheduler locks on the canonical Job and Orchestration storage directories before recovery/admission; a second execution host exits clearly until restart reclaim and durable capacity accounting replace this gate. Concurrent controller sessions can share one foreground HTTP host today, while durable record identity, wait, and cancellation no longer depend on its process-local Promise maps. Invalid `serve` arguments are rejected before runtime construction.

Production record directories now contain `agentknot.sqlite`, the transactional authority for bounded record projections, append-only events, CAS revisions, scoped idempotency keys, fenced execution leases, and durable cancellation intent. Existing per-record JSON snapshots are imported once without being rewritten or deleted. The older hidden lifetime-lock database still temporarily prevents two execution schedulers while restart reclaim is implemented; it is no longer the record/event authority. TypeScript callers must call `await runtime.close()` after admitted work settles. Current startup still fails prior nonterminal work without replay after taking the transitional lock, so full restart recovery is not yet claimed ([decision 0055](postmortems/0055-durable-middleware-kernel.md)).
...
Durable leaf admission is one transaction containing the queued state, `job.queued` event, optional idempotency identity, and first fenced lease. An admission failure starts no worker and retains no partial identity. If a later event, artifact record, or terminal transition cannot be saved, completion rejects with `JobPersistenceError`; AgentKnot does not retry the worker, synthesize a terminal event, or deliver the callback, and the last successfully persisted projection remains authoritative. An accepted cancellation is the deliberate terminal-race exception: a simultaneous success projection is refused and becomes one cancelled result. An unrecorded patch file is removed together with its managed worktree. Startup reconciliation can still fail a persisted nonterminal record without replaying it until restart recovery lands.

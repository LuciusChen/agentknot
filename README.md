...
`agentknot task` is the smallest WorkOrder-rooted developer loop: it issues an immutable WorkOrder, admits and binds one ordinary Executor Job, waits for completion, and reports the task intent, technical status, result summary, changed paths, worker-reported checks, artifact-integrity outcome, and next action. Default output is for people and omits WorkOrder/Job IDs, artifact paths, hashes, and raw records. `--json` returns the complete WorkOrder, Job, and artifact-verification evidence for automation; its WorkOrder ID can be read again after process restart with `agentknot task-show WORK_ORDER_ID [--json]`. WorkOrders use the sibling `work-orders/` directory next to configured Job storage.
...
The `agentknot task` CLI is a thin controller-side composition of those existing boundaries. It issues the WorkOrder, admits the Job through the ordinary Job API, binds the returned durable Job identity, then reads existing terminal result and artifact evidence. `task-show` repeats only the read side. Human output deliberately separates technical completion, changed paths, unverified worker test claims, and the next controller action; it never presents Job success as acceptance. Complete internal evidence remains available only through the explicit JSON mode. Neither command creates another workflow record or applies the artifact, and the admission/binding crash window remains explicit.
...
| **Current** | Minimal immutable, status-free `CandidateRecord` evidence linking one WorkOrder, its bound Executor Job, and one exact persisted Git-patch identity. | The explicit TypeScript `CandidateService` validates the read-only source projections, then `SqliteCandidateStore` atomically persists the Candidate snapshot and `candidate.created` event through the existing SQLite/event infrastructure. The Candidate has no status and does not mutate WorkOrder/Job records or imply Review, Disposition, acceptance, application, or promotion; no Candidate CLI flow exists. |
...
The minimal Candidate surface is also TypeScript-only: `CandidateService.create` accepts a WorkOrder ID, its bound Executor Job ID, and `{ path, sha256, baseCommit }`, while `CandidateService` and `SqliteCandidateStore` expose read and event-cursor operations. Candidate creation validates exact recorded artifact identity and atomically persists only the Candidate snapshot plus `candidate.created` event; it has no CLI or HTTP endpoint and does not create Review or Disposition, decide acceptance, apply or promote artifacts, or change WorkOrder/Job status.

...
| **Current** | Ordered job/orchestration snapshots and normalized events with retries, timeouts, cancellation, one-shot callbacks, and bounded exact-child Pi supervision. | Implemented and covered by deterministic lifecycle, persistence, callback, and Pi conformance tests; Job admission atomically includes `job.queued`, later store failures are surfaced as control-plane errors without worker retries or fabricated terminal state, callback bookkeeping cannot rewrite an already-persisted result, and custom adapters retain their own termination obligations. |

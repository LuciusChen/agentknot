...
| **Experimental** | Selective same-workspace context references. | A controller may label portable repository-relative candidates as `kind: "workspace-file"`; workers can read only the task-relevant subset with their existing tools, while every other locator remains metadata-only. In three blind pairs per configured route, selective reads preserved exact quality and reduced provider-reported total tokens by 30.13% on DeepSeek Flash/max and 32.66% on Luna/max, but both routes failed the complete latency/traffic promotion gate. No references are generated automatically ([experiment/decision 0081](postmortems/0081-selective-workspace-context-ab.md)). |

...
WorkOrder issue/binding and explicit `CandidateRecord` creation are implemented. `CandidateRecord` is immutable and status-free evidence linking one WorkOrder and its bound Executor Job to one exact persisted Job `git-patch` artifact identity: `path`, `sha256`, and `baseCommit`. `CandidateService` validates read-only WorkOrder/Job projections before writing the Candidate projection; those reads and Candidate persistence are separate stores and are not one cross-store transaction. `SqliteCandidateStore` reuses the existing durable SQLite projection and append-only event-cursor infrastructure, and its creation write includes `candidate.created`. Candidate creation never reads, hashes, copies, or applies patch bytes, and it does not mutate WorkOrder/Job records or statuses. Candidate creation is an explicit TypeScript API only, not a CLI flow. Domain `Review` and `Disposition` remain future work; existing orchestration advisory review is execution evidence and is not that future domain lifecycle.

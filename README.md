...
| **Experimental** | Thin installable Codex and Claude controller plugins with explicit delegation and pre-model automatic entry. | Both repository marketplaces install successfully, their manifests and Skills pass native validators, and deterministic tests cover the shared CLI/evidence boundary, pre-model success/failure hook execution, session-bound explicit repository paths, and either package surviving removal of the other. Malformed or nonzero terminal handoff failures block the submitted prompt with bounded evidence, trigger no preview, and cannot add a fallback route/model call. A real fresh Chirp Codex process exposed the former dirty-workspace admission gap; worktree mode now snapshots supported staged, unstaged, and non-ignored untracked state without changing the source. Real Codex experiments cover planner failure with zero children and a deterministic 500 ms Pi route timeout whose exact PID and managed worktree were gone before controller continuation. A read-only direct comparison reduced upstream input by 88.5%. A separate non-empty implementation comparison reduced Codex input from 2,266,538 on the controller-first/manual-delegation path to 141,781 with pre-model dispatch (93.7%); this second baseline is not a pure direct run. One later Codex run proved automatic two-child dispatch, disjoint verified artifacts, and upstream integration, but intentionally added no costly direct baseline. Real Claude parity remains a promotion gate. The separately installed `agentknot` CLI is required ([decisions 0027](postmortems/0027-controller-native-integration-boundary.md), [0029](postmortems/0029-controller-cli-and-single-child-delegation.md), [0030](postmortems/0030-pre-model-controller-dispatch.md), [incident 0031](postmortems/0031-bounded-pi-output-drain.md), [experiment 0032](postmortems/0032-pre-model-multi-child-evidence.md), [incident 0033](postmortems/0033-controller-timeout-phase-claim.md), [decision 0045](postmortems/0045-controller-session-workspace-binding.md), [incident/decision 0048](postmortems/0048-controller-hook-failure-blocking.md), and [decision 0049](postmortems/0049-dirty-workspace-snapshot-isolation.md)). |
| **Current** | Independent worker/provider/model routing with built-in Mock, Pi RPC, and OpenCode JSON adapters. | Routing and core Job semantics are adapter-neutral. Planner, worker, and reviewer targets may be complete-route pools; the repository's logical advanced, routine, and review pools each contain multiple replaceable Pi/native-OpenCode candidates. |
...
| **Current** | Ordered job/orchestration snapshots and normalized events with retries, timeouts, cancellation, one-shot callbacks, and bounded exact-child Pi supervision. | Catchable server shutdown rejects new admissions but remains reachable while admitted work cancels and drains; it closes the listener before releasing storage locks. Late attempt events are ignored, and the bounded Stage 1 soak verifies exact process-group cleanup. |
...
| **Current** | Product-owned local service discovery for one exact `127.0.0.1` `serve` process. | After listen, the server publishes one per-user record that later CLI commands and Codex/Claude hooks discover; `agentknot client --json` reports `unconfigured`, `available`, or `unavailable`. Deterministic verification passes 250 of 250, including cross-process, wait/reconnect, and hook-parity gates; stale or malformed records do not trigger local or model fallback. |
...
| **Current** | Native OpenCode JSON worker as a replaceable pool member. | The pinned `v1.18.15` adapter passes deterministic protocol/lifecycle coverage plus repeated real Luna/max success, error/nonzero, cancellation, timeout, cleanup, and non-empty artifact evidence. Native OpenCode may participate in planner, routine, advanced, and review pools through configuration; no efficiency, capacity, ranking, quota, or fallback claim is made ([decisions 0041](postmortems/0041-native-opencode-worker-portability.md), [0042](postmortems/0042-complete-route-pool-balancing.md), [0043](postmortems/0043-native-opencode-lifecycle-soak.md), and [0047](postmortems/0047-resumable-controller-binding-and-replaceable-role-pools.md)). |
...
The repository configuration dogfoods `mode: "auto"` with replaceable logical targets: `advanced-workers` for planning and medium/high/default children, `routine-workers` for low children, and `review-workers` for advisory review. These pools currently span Pi and native OpenCode complete routes, and their membership—not core code—selects provider/model/effort candidates. Least-active admission counts every active exact member Job and rotates equal-load ties; each Job persists its logical target and selected immutable exact route. Retries stay on that route and never become fallback. The product defaults are `maxChildren: 2` and `maxConcurrency: 2` when dispatch limits are omitted; this repository keeps a six-task pool with six active execution slots. Formal six-child orchestrations at concurrency four, five, and six previously succeeded through Pi/OpenCode Go/Luna/max; at six, all children started within 44 ms and completed on their first attempt. Six is repository evidence, not universal capacity. The scheduler starts only useful available tasks and refills slots as workers complete. The planner returns a strict assessment and marks work parallel only when subtasks are independently verifiable, dependency-free, and non-overlapping. A bounded substantive task may become one non-parallel child. Concrete `repository-analysis` that searches, compares, or interprets project content and returns independently verifiable findings is delegation-first even when read-only and low-complexity; its subtask must name the execution workspace, references, exact scope, and non-goals, then return only decision-relevant deltas with at most five findings and 4,000 characters. Repository inventories, exhaustive summaries, and source restatement are excluded unless explicitly requested. Retrieving one explicit fact from one already identified location may remain upstream. Product decisions, artifact integration, commits, and pushes remain upstream ([decisions 0035](postmortems/0035-delegation-first-small-repository-deliverables.md), [0047](postmortems/0047-resumable-controller-binding-and-replaceable-role-pools.md), [0051](postmortems/0051-evidence-producing-repository-analysis.md), and [0052](postmortems/0052-bounded-analysis-and-observable-waiting.md)).
...
In discovered or explicitly selected server mode the CLI and both hooks submit, wait, cancel, list, verify, and preview through that server. Remote execution no longer polls a growing full Job or Orchestration record every 100 ms. A `/wait` request is held by the middleware until terminal completion or a five-second compact heartbeat; heartbeats contain only phase, route/status, and last-activity identity, while the terminal response remains authoritative. `--progress` prints connected phase/activity evidence to stderr, labels transport loss as `disconnected`, and retries the same durable ID at most three times without resubmitting work. The hook forwards that detail to its expandable execution record; its host status line remains one static summary because no portable dynamic-status contract is assumed. The clients do not open local storage, run startup reconciliation, or discover a repository-local AgentKnot configuration. The server owns the shared task pool and configured concurrency once, so more upstream sessions do not manufacture extra worker capacity. `--config PATH` remains the deliberate local override; otherwise `--server URL` and `AGENTKNOT_SERVER_URL` are explicit server overrides, with `--config` and `--server` mutually exclusive. An explicitly set `AGENTKNOT_CONFIG` remains the existing local opt-in. `doctor` and `usage` stay local and are not redirected through discovery. Server failure is explicit and never falls back to a local runtime or another model ([incident/decision 0052](postmortems/0052-bounded-analysis-and-observable-waiting.md)).
...
After installation or any hook change, review and trust the plugin hook in the controller's native hook UI, then start or resume the controller in a new process. The shared dependency-free hook accepts a bounded adapter-provided source namespace and explicit-invocation marker; Codex and Claude are packaged adapters, not core branches. A resumed session whose retained command predates the marker argument is handled by a bounded compatibility path rather than failing before hook error containment. The pre-model path first uses the event cwd's Git root. From a non-Git cwd, one unambiguous explicit existing absolute or `~/...` path may establish the root; after a tool call, one structurally supplied `cwd`, `workdir`, or `workspace` Git root becomes the latest session focus for the next prompt. Multiple roots are ignored, and command strings, tool output, transcripts, and home are never parsed or scanned. A schema-versioned 0600 record keyed by source and `session_id` preserves the workspace plus a hashed Git common-directory identity for continuation, project switching, and exit/resume. `SessionEnd` retains it, and every reuse revalidates both so a different repository recreated at the same path is rejected. Valid cwd/path/tool evidence overwrites the binding; invalid records are removed. The hook invokes ordinary orchestration before the controller-model request; planner, child, and reviewer work target configured logical route pools, and the adapter never chooses a runtime or model. The planner receives the exact submitted text, not the controller conversation. If that text alone stays upstream but the controller already has enough surrounding context to recover one concrete bounded repository task, the hook now permits the controller to submit that recovered task once through the normal AgentKnot entry before doing its repository work. It never reads a transcript or treats a path alone as task semantics ([decisions 0045](postmortems/0045-controller-session-workspace-binding.md), [0047](postmortems/0047-resumable-controller-binding-and-replaceable-role-pools.md), and [0050](postmortems/0050-context-dependent-continuation-handoff.md)).

For delegated work, the hook supplies compact terminal evidence and integrity-valid non-empty patch previews but never applies them. All child outputs share a 24,000-character budget, previews share 32,000 characters, and total model-visible hook context is capped at 60,000 characters. A configured `auto` repository therefore forwards every non-explicit submitted prompt to the configured planner, including prompts the planner later retains upstream; use `off` or `suggest` where that latency or data boundary is unacceptable. If discovery, CLI execution, or handoff parsing fails, the hook blocks that submitted prompt before the controller-model request and reports a bounded reason instead of silently repeating the work upstream. A nonzero orchestration exit retains only its bounded terminal ID, status, and error for that report. Resolve the prerequisite and retry; an intentional planner decision to keep the exact submitted text upstream remains normal and non-blocking. The controller-context recovery above is conditional and post-model: it must reconstruct a different self-contained task, runs normal policy again, and does not claim the full pre-model token savings of a self-contained prompt. No MCP server, wrapper daemon, transcript parser, local semantic classifier, learned router, or fallback model is added; see [decisions 0030](postmortems/0030-pre-model-controller-dispatch.md), [0048](postmortems/0048-controller-hook-failure-blocking.md), and [0050](postmortems/0050-context-dependent-continuation-handoff.md).
...
`GET /v1/jobs/:id/wait` and `GET /v1/orchestrations/:id/wait` return the terminal record immediately when available. While the exact execution is still active on this server they return `202` after five seconds with a compact progress object; a nonterminal record that is not active on this server returns `409` rather than pretending it is merely waiting. CLI `run` and `orchestrate` use this transport automatically in server mode.
...
```json
{
  ...
  "delegation": {
    "mode": "auto",
    "planner": { "strategy": "hybrid", "route": "advanced-workers" },
    "dispatch": {
      "defaultRoute": "advanced-workers",
      "maxChildren": 6,
      "maxDepth": 1,
...
Use `--config PATH` or `AGENTKNOT_CONFIG` for another configuration file. Route and worker names above describe only this dogfood deployment: controller identity, planner, worker, reviewer, adapter, provider, model, and effort are not fixed in core. A leaf request, dispatch default/rule, planner, or quality-review target may name a pool; top-level default and `doctor` remain exact-route surfaces. Pools contain 2–20 unique exact routes, use process-local `least-active` with rotating equal-load ties, and never switch a selected Job during retry. Every candidate in a quality-review pool must have `maxAttempts: 1`; omission disables review. `artifactValidation` is also optional and remains bounded/shell-free as configured. JSON configuration selects the built-in `mock`, `pi-rpc`, and `opencode-json` adapters; custom TypeScript adapters remain available through direct construction.
...
Stage 1's dependable local job loop is complete: the deterministic suite and bounded host soak cover lifecycle, catchable shutdown, fail-without-replay runtime recovery, exact-child cleanup, worktree cleanup, persistence bounds, and artifact handoff, with real Pi and native OpenCode evidence. Controller conversations can resume their verified workspace binding; interrupted Jobs themselves are still failed rather than replayed. Current limits include one conforming file-runtime writer, depth one, no automatic artifact integration, no semantic verification of captured paths, and no authentication for the local HTTP service.

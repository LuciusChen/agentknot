...
The promoted real worker adapter uses [Pi RPC](https://pi.dev/docs/latest/rpc), a strict JSONL protocol. An experimental second real adapter invokes OpenCode CLI's JSON run surface directly, proving that Pi is replaceable without changing Job semantics. Neither path fixes provider, model, or effort in core.
...
| **Current** | Independent worker/provider/model routing with built-in Mock, Pi RPC, and OpenCode JSON adapters. | Routing and core Job semantics are adapter-neutral; the promoted planner/default remains Pi/OpenCode Go/Luna/max and the low-complexity rule remains Pi/OpenCode Go/DeepSeek Flash/max. The native OpenCode route is manual and experimental. |
...
| **Experimental** | Native OpenCode JSON worker and manually selected `opencode-luna` route. | The pinned `v1.18.15` adapter passes deterministic conformance, strict protocol, cancellation/timeout/cleanup, diagnostic, usage, completion-report, and worktree-artifact tests. Exact-route Luna/max live inference and one real isolated Job succeeded with the independent OpenCode auth store. Pi remains the promoted default until repeated real failure/cancellation/timeout soaks close the remaining gate ([decision 0041](postmortems/0041-native-opencode-worker-portability.md)). |
...
`OpenCodeJsonWorkerAdapter` invokes `opencode run --pure --format json` and passes the exact resolved `provider/model`, optional thinking variant, and isolated worktree path. It shares the strict JSONL decoder, completion-report parser, and bounded exact-child supervision with Pi, while success is inferred from a valid `step_finish` plus clean process exit because OpenCode exposes no `agent_settled`. Completed text/tool parts have lower lifecycle fidelity than Pi and are not described as token streaming or tool-start/update events. Exact `step_finish` usage is retained in the same route-neutral usage shape. The adapter uses OpenCode's own private auth store or explicit required environment; `unsetEnvironment` can remove an ambient key and never reads Pi auth. `--pure` disables plugins, but OpenCode's data directory still owns auth/session state, so this is not full data isolation or a sandbox ([decision 0041](postmortems/0041-native-opencode-worker-portability.md)).

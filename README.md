...
| **Experimental** | Thin installable Codex and Claude controller plugins with explicit delegation and pre-model automatic entry. | Deterministic tests cover package independence, shared CLI/handoff behavior, workspace binding, dirty-workspace isolation, and blocking failures without fallback. The separately installed `agentknot` CLI is required, and real Claude parity remains the promotion gate. Detailed experiments remain in [decisions 0027–0050](postmortems/README.md). |
...
The repository configuration dogfoods `mode: "auto"` with replaceable logical targets: `advanced-workers` for planning and medium/high/default children, `routine-workers` for low children, and `review-workers` for advisory review. Pool membership—not core code—selects provider/model/effort candidates. Least-active admission snapshots one exact route per Job, and retries never become fallback. Product defaults are two children and two active slots; this repository uses six. The scheduler starts only useful independent tasks, refills free slots, and never treats the configured maximum as a target.

The planner keeps product decisions, artifact integration, commits, and pushes upstream. It delegates bounded repository deliverables and evidence-producing analysis, including a single non-parallel task, while preserving one authoritative writable target and bounded read-only references. The normative classification, scope, and output rules live in [the technical specification](docs/SPEC.md).

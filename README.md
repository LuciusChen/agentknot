...
`agentknot task` is the smallest WorkOrder-rooted developer loop: it issues an immutable WorkOrder, admits and binds one ordinary Executor Job, waits for completion, and reports the task intent, technical status, result summary, changed paths, worker-reported checks, artifact-integrity outcome, and next action. Default output is for people and omits WorkOrder/Job/Candidate IDs, artifact paths, hashes, and raw records. `--json` returns complete machine evidence; its WorkOrder ID can be read again after process restart with `agentknot task-show WORK_ORDER_ID [--json]`. After a successful execution, `agentknot task-candidate WORK_ORDER_ID [--json]` explicitly records or reloads the immutable Candidate for the exact verified terminal-attempt artifact. WorkOrders and Candidates use sibling `work-orders/` and `candidates/` directories next to configured Job storage.
...
The minimal `ReviewRecord` is also implemented as an explicit TypeScript-only domain API. `ReviewService.create` checks that one `CandidateRecord` exists through a read-only Candidate store, then `SqliteReviewStore` atomically persists one immutable schemaVersion 1, status-free Review snapshot plus one `review.created` event in its separate SQLite store. The record links only to `candidateId`, retains bounded non-empty `reviewer` and `summary` values, and defensively copies at most ten structured findings with independent `low`/`medium`/`high` severity, bounded non-empty `message`, and bounded non-empty `evidence` fields. It has no verdict, recommendation, acceptance/discard value, lifecycle status, or `updatedAt`; multiple independent reviews may reference one Candidate. Candidate validation and Review persistence are separate stores rather than a cross-store transaction, and no recovery or reconciliation is added. Existing orchestration advisory-review verdicts/findings are a separate execution contract and are never imported or converted. Review has no CLI/HTTP workflow, reviewer launching, automatic review, source-record transition, Disposition, acceptance, discard, artifact application, commit, merge, push, or promotion behavior.
...
Use `agentknot task` for the minimal WorkOrder-rooted developer loop, `agentknot run` for an already bounded leaf Job without a WorkOrder, or `agentknot orchestrate` with a controller-authored assessment when deterministic policy should decide whether and how to dispatch it.
...
The minimal Review surface is TypeScript-only: `ReviewService.create` accepts one existing Candidate ID, a bounded non-empty reviewer identity and summary, and bounded structured findings; `ReviewService` and `SqliteReviewStore` expose create/read/list/event-cursor operations. Review creation validates Candidate existence without mutating it, atomically persists only the Review snapshot plus `review.created` event, rejects general save, and does not read or change WorkOrder, Job, or Orchestration records or statuses. Its validation and persistence are separate stores without cross-store transaction, recovery, or reconciliation. It has no CLI or HTTP endpoint and does not consume existing orchestration quality-review verdicts/findings.

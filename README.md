...
npm run build
npm test
node dist/src/cli.js task --route mock --source codex --workspace . \
  --acceptance "the requested change is complete and its evidence is recorded" \
  "Make one bounded change"
```

`agentknot task` is the smallest WorkOrder-rooted developer loop: it issues an immutable WorkOrder, admits and binds one ordinary Executor Job, waits for completion, and reports the task intent, technical status, result summary, changed paths, worker-reported checks, artifact-integrity outcome, and next action. Default output is for people and omits domain IDs, artifact paths, hashes, and raw records. `--json` returns complete machine evidence; its WorkOrder ID can be read again after process restart with `agentknot task-show WORK_ORDER_ID [--json]`. After a successful execution, `agentknot task-candidate WORK_ORDER_ID [--json]` explicitly records or reloads the immutable Candidate for the exact verified terminal-attempt artifact. `task-review WORK_ORDER_ID --reviewer NAME --summary TEXT [--finding-json JSON]` records one already-authored Review without launching a reviewer, and `task-reviews WORK_ORDER_ID [--json]` reloads all Reviews after restart. `task-disposition WORK_ORDER_ID --decision accept|discard --decided-by NAME --rationale TEXT [--review REVIEW_ID] [--json]` records the explicit final decision, while `task-dispositions WORK_ORDER_ID [--json]` reloads it without applying or promoting the artifact. These domain records use sibling directories next to configured Job storage.
...
The `agentknot task` CLI is a thin controller-side composition of those existing boundaries. It issues the WorkOrder, admits the Job through the ordinary Job API, binds the returned durable Job identity, then reads existing terminal result and artifact evidence. `task-show` repeats only the read side. Neither command creates a Candidate. The separate `task-candidate` action requires a successful bound Job and an exact terminal-attempt artifact that passes current integrity verification, then calls the existing Candidate domain service; a later sequential call reloads the matching record. Human output deliberately separates technical completion, changed paths, unverified worker test claims, Candidate evidence, and the next controller action while hiding internal identity. No command presents Job success or Candidate creation as acceptance, applies the artifact, or closes the admission/binding and cross-store Candidate persistence windows.

The current minimal domain sequence is deliberately separate from execution lifecycle:

```text
WorkOrder -> Executor Job -> Candidate -> Review -> Disposition
```

WorkOrder issue/binding, explicit `CandidateRecord` creation, and explicit `ReviewRecord` creation are implemented. `CandidateRecord` is immutable and status-free evidence linking one WorkOrder and its bound Executor Job to one exact persisted Job `git-patch` artifact identity: `path`, `sha256`, and `baseCommit`. `CandidateService` validates read-only WorkOrder/Job projections before writing the Candidate projection; those reads and Candidate persistence are separate stores and are not one cross-store transaction. `SqliteCandidateStore` reuses the existing durable SQLite projection and append-only event-cursor infrastructure, and its creation write includes `candidate.created`. Candidate creation never reads, hashes, copies, or applies patch bytes, and it does not mutate WorkOrder/Job records or statuses. The TypeScript API remains the domain authority; `task-candidate` is only an explicit WorkOrder-rooted CLI composition over it and current artifact verification.

The minimal `ReviewRecord` is also implemented as an explicit domain API. `ReviewService.create` checks that one `CandidateRecord` exists through a read-only Candidate store, then `SqliteReviewStore` atomically persists one immutable schemaVersion 1, status-free Review snapshot plus one `review.created` event in its separate SQLite store. The record links only to `candidateId`, retains bounded non-empty `reviewer` and `summary` values, and defensively copies at most ten structured findings with independent `low`/`medium`/`high` severity, bounded non-empty `message`, and bounded non-empty `evidence` fields. It has no verdict, recommendation, acceptance/discard value, lifecycle status, or `updatedAt`; multiple independent reviews may reference one Candidate. Candidate validation and Review persistence are separate stores rather than a cross-store transaction, and no recovery or reconciliation is added. Existing orchestration advisory-review verdicts/findings are a separate execution contract and are never imported or converted. The `task-review` CLI only supplies already-authored evidence to this service, and `task-reviews` is its WorkOrder-rooted read projection; neither launches a Reviewer Job, converts advisory review, transitions a source record, creates Disposition, accepts/discards, or applies/promotes an artifact.

The minimal `DispositionRecord` is implemented as an explicit final controller decision. `DispositionService.record` validates one existing Candidate and one existing Review belonging to it through separate read-only stores, then `SqliteDispositionStore` atomically persists one immutable schemaVersion 1, status-free record plus one `disposition.recorded` event. Its fixed-length SHA-256 Candidate-derived ID allows exactly one disposition per Candidate; exact semantic replay returns the original without a second event, while a different Review ID, decision, `decidedBy`, or rationale raises an explicit conflict, including concurrent attempts from independent stores. The record stores only `accept` or `discard`, bounded non-empty `decidedBy` and rationale, and `createdAt`; its decidedBy/rationale fields use fixed UTF-8 bounds and the durable projection uses the existing 16 MiB record budget. WorkOrder-rooted `task-disposition` supplies this decision explicitly, selects the sole Review or requires `--review` when ambiguous, and `task-dispositions` reloads linked decisions. Neither command infers a decision or applies, promotes, commits, merges, pushes, deletes, or otherwise mutates the Candidate artifact or canonical workspace. Job success remains technical execution evidence and is not acceptance; there is no Disposition HTTP workflow or automatic disposition.
...
Use `agentknot task` for the minimal WorkOrder-rooted developer loop, `task-candidate`, `task-review`, and `task-disposition` for its explicit evidence and decision steps, `agentknot run` for an already bounded leaf Job without a WorkOrder, or `agentknot orchestrate` with a controller-authored assessment when deterministic policy should decide whether and how to dispatch it.
...
| --- | --- | --- |
| **Current** | Durable immutable WorkOrders with explicit binding to one already admitted executor Job identity, plus a minimal developer task CLI. | Issue persists the command and `work-order.issued` atomically. Binding uses the WorkOrder event sequence as expected revision, appends one `work-order.executor-job.bound` event, treats the same Job as an idempotent replay, and rejects stale first binds or a different Job. `agentknot task` composes issue, ordinary Job admission, binding, wait, and a human completion report; `task-show` reloads that evidence after restart. Internal IDs and artifact metadata require explicit `--json`. Neither command changes domain status or promotes artifacts. |
| **Current** | Minimal immutable, status-free `CandidateRecord` evidence linking one WorkOrder, its bound Executor Job, and one exact persisted Git-patch identity. | The explicit `CandidateService` validates the read-only source projections, then `SqliteCandidateStore` atomically persists the Candidate snapshot and `candidate.created` event through the existing SQLite/event infrastructure. `task-candidate` adds one explicit WorkOrder-rooted presentation path that requires a successful Job and current exact artifact verification before creation, reuses a matching record on later sequential calls, and hides internal identity outside `--json`. The Candidate has no status and does not mutate WorkOrder/Job records or imply Review, Disposition, acceptance, application, or promotion. |
| **Current** | Minimal immutable, schemaVersion 1, status-free `ReviewRecord` evidence linked only to one existing CandidateRecord. | `ReviewService.create` validates Candidate existence read-only and `SqliteReviewStore` atomically persists one snapshot plus `review.created` event in a separate store. WorkOrder-rooted `task-review` records already-authored reviewer/summary/findings input and `task-reviews` reloads it with human-default or complete JSON presentation; multiple Candidates require explicit selection rather than guessing. The record has no verdict, recommendation, acceptance/discard value, lifecycle status, or `updatedAt`, multiple reviews per Candidate are allowed, existing orchestration quality-review evidence is not converted, and no Reviewer Job or automatic review is added. |
| **Current** | Minimal immutable, schemaVersion 1, status-free `DispositionRecord` final controller decision linked to one Candidate and one belonging Review. | `DispositionService.record` validates both source records read-only and `SqliteDispositionStore` atomically persists one fixed-length Candidate-derived identity plus `disposition.recorded`. WorkOrder-rooted `task-disposition` records explicit accept/discard input with exact replay/conflict semantics, and `task-dispositions` reloads it with human-default or complete JSON presentation. The record and commands never apply or promote an artifact, change a lifecycle status, infer a decision, or treat Job success as acceptance; no Disposition HTTP workflow exists. |
...
| **Experimental** | Selective same-workspace context references. | A controller may label portable repository-relative candidates as `kind: "workspace-file"`; the worker prompt asks for task-relevant subset reads with existing tools, while every other locator remains metadata-only. In three blind pairs per configured route, selective reads preserved exact quality and reduced provider-reported total tokens by 30.13% on DeepSeek Flash/max and 32.66% on Luna/max, but both routes failed the complete latency/traffic promotion gate. A later real-repository DeepSeek probe used 5.55% and 23.21% more tokens in two reference variants and remained slower, while ordinary tools could still search beyond the candidates. No references are generated automatically ([experiments/decisions 0081](postmortems/0081-selective-workspace-context-ab.md) and [0082](postmortems/0082-real-repository-selective-context-gate.md)). |
...
| **Current** | Read-only persisted-evidence usage report through CLI and TypeScript. | `agentknot usage` and `runtime.usage()` aggregate fixed-shape downstream evidence once per worker attempt, including failed and retried attempts when statistics are available; legacy successful Jobs fall back to terminal metadata. The report keeps unavailable attempts explicit and reports cache-read plus active/shadow route-rule hit rates. Upstream controller usage, cross-boundary proportions, and shared-account remaining quota remain unavailable rather than inferred ([decisions 0034](postmortems/0034-persisted-usage-observability-boundary.md) and [0086](postmortems/0086-per-attempt-usage-evidence.md)). |
| **Current** | Ordered job/orchestration state and normalized events with retries, timeouts, cancellation, one-shot callbacks, and bounded exact-child Pi supervision. | Snapshot projection and event suffix commit atomically; stale revisions and stale lease fences cannot publish over current work. A worker adapter owns retry inside its live downstream session; when that session settles failed after its own policy, `WorkerSettledError` prevents AgentKnot from immediately replaying the whole Job. Structured pre-settlement worker failures retain the configured exact-route Job attempt but first persist a bounded exponential delay with deterministic per-Job jitter; cancellation interrupts the wait. No path parses provider strings or silently changes routes ([incident/decisions 0084](postmortems/0084-worker-settled-retry-ownership.md) and [0087](postmortems/0087-pre-settlement-worker-retry-backoff.md)). |
| **Current** | Bounded normal-run Pi text tool results before the next model call. | A product-owned ephemeral extension runs after configured extensions and caps aggregate text to 8 KiB with explicit UTF-8-safe evidence; Bash keeps the tail while other tools keep the head. The exact artifact-read capability is exempt. One same-task Luna/max probe reduced reported total tokens 15.1% and cost 23.9% but increased latency 35.5%, so this is a fixed context-safety boundary with one positive token result, not a universal efficiency claim ([incident/decision 0088](postmortems/0088-bound-pi-tool-result-context.md)). |
| **Current** | Compact route-neutral worker activity on cursor waits. | Existing durable events are projected into lifecycle state, explicit complete/partial/truncated coverage, last-observation age, bounded sanitized active-tool names, and retry scope/progress for downstream calls or completion-envelope recovery. Pi text frames are coalesced at its adapter boundary so provider chunking does not crowd out later tool evidence; raw retry errors and recovery prompts remain out of the compact projection, client disconnect remains separate, and silence is never called provider loss ([decisions 0072](postmortems/0072-compact-worker-activity-and-pi-frame-coalescing.md), [0084](postmortems/0084-worker-settled-retry-ownership.md), and [0085](postmortems/0085-in-session-completion-envelope-recovery.md)). |
...
| **Current** | Additive terminal Job completion summaries and required real-worker completion reports. | Newly terminal success, failure, and cancellation records include terminal outcome/attempt, controller-captured terminal-artifact path evidence or a stable unavailable reason, and an explicit worker-reported reported/unavailable branch. After a normal Pi turn settles without a valid strict envelope, its adapter asks once for only that protocol evidence through the same live process/session; success stays on the same Job attempt, while a second invalid settlement fails non-retryably instead of repeating the task. A valid `taskOutcome: blocked` report likewise fails once and remains visible. Custom TypeScript adapters may still omit the optional report, while every accepted report remains an unverified worker claim ([incident/decisions 0044](postmortems/0044-required-worker-completion-and-canonical-worktree-id.md), [0069](postmortems/0069-repeated-shared-context-scope-trials.md), and [0085](postmortems/0085-in-session-completion-envelope-recovery.md)). |
...
For repository work the assessment may include one optional `context` manifest: a summary, repository-relative initial paths, hard constraints, and optional references already known by the controller. A reference identifies a locator and source, may carry a revision, SHA-256 digest, and summary, and is always marked `unverified`. The experimental `workspace-file` kind requires a portable repository-relative locator and tells a worker to read only the task-relevant subset with its existing workspace tools; it grants no new tool authority, prevents no ordinary listing/search, verifies no file, and makes no sandbox claim. Other kinds remain metadata-only and must not be resolved. The complete UTF-8 manifest is capped at 2 KiB and becomes the same prefix for every selected child before that child's own instruction. Explicit constraints override generic check guidance; explicit task output formats override the generic completion prose request; insufficient context is reported with available evidence instead of authorizing wider exploration. Do not include transcripts, file contents, repository inventories, credentials, or model/provider data, and omit the manifest rather than scanning the repository to construct it. Blind five-candidate A/B trials established repeated total-token reductions but failed the complete latency/traffic gate, and a later real-repository probe rejected automatic reference generation after both variants regressed tokens and latency ([experiment 0069](postmortems/0069-repeated-shared-context-scope-trials.md), [decision 0077](postmortems/0077-task-context-reference-manifest.md), and [experiments/decisions 0081](postmortems/0081-selective-workspace-context-ab.md) and [0082](postmortems/0082-real-repository-selective-context-gate.md)).
...
Controller Skills prefer the common MCP admission/status/preview tools. The transport-equivalent `agentknot orchestrate --handoff-json` remains a fallback where MCP is unavailable. Both return a summary-first projection of persisted status, action, route evidence, child IDs, bounded completion evidence, output availability/size, compact errors, parent artifact review, and artifact verification while omitting Worker output, event history, policy snapshots, repeated prompts, and execution prompts. The complete serialized projection has one non-configurable 32 KiB UTF-8 budget, so its default and maximum are both 32 KiB. If local field limits are not enough, AgentKnot removes optional presentation detail in a fixed order while retaining every child's identity, status, route identity, completion outcome, output readability/truncation evidence, artifact final status, and minimum failure evidence. `handoffTruncation` reports the original size, budget, omission count, and affected child IDs.

Worker output remains in the durable Job and Orchestration records under the existing 1 MiB retained-result boundary; handoff construction does not delete or further truncate it. Handoff `outputBytes` is the UTF-8 byte size actually retained in `JobRecord.result.output`, and `outputTruncated` means the earlier durable-retention boundary discarded an original suffix. Pagination cannot recover that discarded suffix. When the summary is insufficient, a controller may call read-only MCP tool `agentknot_job_output`, TypeScript `readJobOutput`, or `GET /v1/jobs/:id/output`. The reader uses a UTF-8 byte cursor, defaults to 4 KiB, accepts 4 bytes through a strict 16 KiB maximum, returns `nextCursor` plus `hasMore`, and explicitly distinguishes missing Jobs, mismatched subtasks, and unavailable output. The 32 KiB handoff budget controls serialized controller context size; it does not prove lower total tokens, cost, or downstream work.
...
`doctor --live` performs one bounded real inference probe with the exact selected worker, provider, model, and thinking level. Its 30-second control-plane timer triggers cooperative abort, and the supported Pi adapter supervises its child process, waits for cleanup, and removes an isolated temporary diagnostic workspace before returning. The command does not fall back to another route, returns the provider error and a nonzero exit status when inference is unavailable, and reports an honest unsupported result when the selected adapter has no probe capability. It creates no Job or artifact record, and normal `run` or orchestration execution does not perform an extra probe. A successful probe is point-in-time evidence for that exact route and does not guarantee a later Job request: the probe and Job execution are separate downstream requests, so either may transiently succeed while the other fails.
...
The same projection is available as `await runtime.usage()`. New Jobs contribute one fixed-shape observation per completed worker attempt, so available usage from failed or retried attempts is not hidden by the final Job result. Each observation contains only input/output/cache-read/cache-write/total fields and provider-reported numeric cost, or one explicit unavailable reason; successful legacy Jobs without attempt evidence still fall back to terminal `result.metadata.sessionStats`. Attempt evidence is authoritative when present, so terminal metadata is never counted again. The human report shows available/observed attempt coverage. A valid all-zero record stays valid; missing, timed-out, unsupported, malformed, unsafe, process-failed, or aggregate-overflow data never becomes zero. The cache-read hit rate is calculated after aggregation as `cacheRead / (input + cacheRead)`, excluding output and cache-write tokens. Routing also reports classified persisted pool selections grouped by logical pool and exact member, so configured route distribution is visible without inferring it from provider text ([decision 0086](postmortems/0086-per-attempt-usage-evidence.md)).
...
Quality-review counts likewise come only from terminal records whose immutable policy configured a reviewer. The report groups completed, skipped, and unavailable outcomes, strict verdicts, finding severities, reviewer route names, and stable skip/failure reasons. The usage projection does not read the separate TypeScript-only Disposition store, so its controller-disposition field remains unavailable with reason `controller-review-disposition-not-persisted`; an explicit `DispositionRecord` is the final recorded Candidate decision, not an artifact application or promotion.
...
Use `--config PATH` or `AGENTKNOT_CONFIG` for another configuration file. Route and worker names above describe only this dogfood deployment: controller identity, worker, reviewer, adapter, provider, model, and effort are not fixed in core. Work scope comes from the controller-authored context, relevant paths, constraints, task prompt, non-goals, and acceptance criteria; AgentKnot does not turn a fixed tool-call count into a task boundary. Timeout, cancellation, bounded records/events/output, isolation, and required completion evidence remain operational failure boundaries rather than complexity estimates ([decision 0083](postmortems/0083-remove-tool-count-task-boundaries.md)). A leaf request, dispatch default/rule, or quality-review target may name a pool; top-level default and `doctor` remain exact-route surfaces. Pools contain 2–20 unique exact routes, use `least-active` with rotating equal-load ties, and never switch a selected Job during retry. Production SQLite admission derives activity from unexpired exact-route Job leases and commits the choice, Job, first lease, and cursor in one transaction; memory and legacy file stores keep local test/migration behavior. Every candidate in a quality-review pool must have `maxAttempts: 1`; omission disables review. `artifactValidation` is also optional and remains bounded/shell-free as configured. JSON configuration selects the built-in `mock` and `pi-rpc` adapters; custom TypeScript adapters remain available through direct construction.
...
The minimal Candidate domain surface remains `CandidateService.create`: it accepts a WorkOrder ID, its bound Executor Job ID, and `{ path, sha256, baseCommit }`, while `CandidateService` and `SqliteCandidateStore` expose read and event-cursor operations. Candidate creation validates exact recorded artifact identity and atomically persists only the Candidate snapshot plus `candidate.created` event. `agentknot task-candidate WORK_ORDER_ID [--json]` is the one CLI composition: it selects the recorded terminal-attempt artifact from a successful bound Job, requires current exact verification before the first write, and sequentially reloads an already matching Candidate. It does not create Review or Disposition, decide acceptance, apply or promote artifacts, or change WorkOrder/Job status. Candidate uniqueness is not a cross-caller transaction; concurrent first calls remain capable of creating more than one immutable record and are not presented as exactly-once.

The minimal Review domain surface remains `ReviewService.create`: it accepts one existing Candidate ID, a bounded non-empty reviewer identity and summary, and bounded structured findings; `ReviewService` and `SqliteReviewStore` expose create/read/list/event-cursor operations. Review creation validates Candidate existence without mutating it, atomically persists only the Review snapshot plus `review.created` event, rejects general save, and does not read or change WorkOrder, Job, or Orchestration records or statuses. `agentknot task-review WORK_ORDER_ID --reviewer NAME --summary TEXT [--finding-json JSON] [--candidate CANDIDATE_ID] [--json]` explicitly records one new Review, while `task-reviews WORK_ORDER_ID [--json]` reads all Reviews linked through that WorkOrder's Candidates. The normal single-Candidate path needs no internal ID; ambiguous multiple Candidates are rejected unless selected explicitly. The CLI does not launch a reviewer or consume orchestration quality-review verdicts/findings, and it adds no verdict, Disposition, acceptance, application, or promotion behavior.

The minimal Disposition domain surface remains `DispositionService.record`: it accepts one existing Candidate ID, one existing belonging Review ID, `accept` or `discard`, and bounded non-empty `decidedBy` and rationale; `DispositionService` and `SqliteDispositionStore` expose record/create, get, getForCandidate, list, and event-cursor operations. Creation validates the two source records read-only, uses one deterministic fixed-length Candidate-derived identity, atomically persists one `disposition.recorded` event, and rejects general save or unsupported schema versions. `agentknot task-disposition WORK_ORDER_ID --decision accept|discard --decided-by NAME --rationale TEXT [--review REVIEW_ID] [--json]` explicitly records or replays that decision, while `task-dispositions WORK_ORDER_ID [--json]` reads linked decisions after restart. A unique Review is selected automatically and an ambiguous set requires explicit selection. The commands do not consume orchestration advisory verdicts, infer a decision, change source records or statuses, apply/promote/mutate artifacts or the canonical workspace, or add HTTP, automatic disposition, commit, merge, or push behavior.

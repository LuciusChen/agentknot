...
| **Current** | Compact route-neutral worker activity on cursor waits. | Existing durable events are projected into lifecycle state, explicit complete/partial/truncated coverage, last-observation age, bounded sanitized active-tool names, and downstream retry scope/progress such as `2/3`. Pi text frames are coalesced at its adapter boundary so provider chunking does not crowd out later tool evidence; raw retry errors remain out of the compact projection, client disconnect remains separate, and silence is never called provider loss ([decisions 0072](postmortems/0072-compact-worker-activity-and-pi-frame-coalescing.md) and [0084](postmortems/0084-worker-settled-retry-ownership.md)). |

...
| **Experimental** | Thin installable Codex and Claude controller clients with explicit or implicit controller-authored handoff. | Each package exposes a controller-native Skill and launches the common `agentknot mcp` stdio client. No per-prompt hook injects hidden context or owns repository/session/runtime state. The controller owns planning and submits a strict assessment through common MCP tools or the transport-equivalent CLI fallback. Deterministic tests cover package parity, broker restart, and the absence of runtime ownership; real Claude parity remains a promotion gate ([decisions 0053](postmortems/0053-controller-owned-planning-handoff.md), [0057](postmortems/0057-independent-broker-and-thin-controller-clients.md), and [0063](postmortems/0063-remove-per-prompt-controller-obligations.md)). |
...
For repository work the assessment may include one optional `context` object: a summary, repository-relative initial paths, and hard constraints already known by the controller. Its complete UTF-8 JSON is capped at 2 KiB and becomes the same prefix for every selected child before that child's own instruction. It is navigation guidance, not trusted evidence or persistent worker memory. Explicit constraints override the generic request to run checks; insufficient context is reported with available evidence instead of authorizing wider exploration. Do not include transcripts, file contents, repository inventories, or model/provider data, and omit it rather than scanning the repository to construct it. Six post-correction Luna/DeepSeek trials across different repository surfaces stayed within every named path and command boundary, but this remains behavioral evidence rather than a sandbox guarantee ([experiment 0069](postmortems/0069-repeated-shared-context-scope-trials.md)).
...
Invoke the full review workflow explicitly with `$agentknot-delegate`, or let the controller invoke the Skill implicitly for eligible repository work.
...
After installation or an MCP/Skill change, review and trust the package in the controller UI, then start a new controller process so it loads the new tools. The packages install no `UserPromptSubmit` hook and inject no repeated developer context. New and resumed sessions use the same controller-native Skill; the upstream controller supplies the actual workspace through the common handoff.

The upstream controller owns eligibility, planning, decomposition, acceptance criteria, and promotion, then calls `agentknot_broker_status`; a stopped or unavailable configured broker gets one explicit `agentknot_broker_start` attempt before policy lookup and eligible admission. Admission is non-blocking; `agentknot_orchestration_follow` performs one bounded durable cursor wait, `agentknot_orchestration_status` remains an immediate snapshot read, and `agentknot_artifact_preview` is read-only. `agentknot mcp` is a broker client and lifecycle control surface, never a runtime owner. AgentKnot owns validation, deterministic policy, routing, scheduling, isolation, recovery, completion, and artifact evidence ([decisions 0053](postmortems/0053-controller-owned-planning-handoff.md), [0057](postmortems/0057-independent-broker-and-thin-controller-clients.md), [0058](postmortems/0058-controller-neutral-broker-activation.md), [0062](postmortems/0062-durable-event-subscription.md), and [0063](postmortems/0063-remove-per-prompt-controller-obligations.md)).
...
Read-oriented CLI commands, including `show`, lists, artifact inspection, route and delegation inspection, and both doctor modes, open persisted records without scheduler ownership or startup recovery. A TypeScript runtime created with `reconcileOnStartup: false` has the same read-only capability boundary: its execution and recovery methods refuse calls. Local `run`, local `orchestrate`, `broker run`, and the deprecated valid `serve` alias acquire transitional scheduler locks on the canonical Job and Orchestration storage directories before recovery/admission; a second execution host exits clearly until the remaining global capacity queue and multi-executor recovery replace this gate. Concurrent controller sessions share one broker, while durable record identity, wait, cancellation, leaf/parent recovery, and route-pool selection do not depend on process-local Promise or activity maps.

...
| **Experimental** | Native OpenCode JSON worker and configured `opencode-luna` pool member. | The pinned `v1.18.15` adapter passes deterministic conformance, strict protocol, cancellation/timeout/cleanup, diagnostic, usage, completion-report, and worktree-artifact tests. Exact-route Luna/max live inference and one real isolated Job succeeded with the independent OpenCode auth store. It now shares medium/high/default child traffic through a human-authored complete-route pool; repeated real pool and native failure/cancellation/timeout soaks remain promotion gates ([decisions 0041](postmortems/0041-native-opencode-worker-portability.md) and [0042](postmortems/0042-complete-route-pool-balancing.md)). |

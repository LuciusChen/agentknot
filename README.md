...
The minimal `DispositionRecord` is implemented as an explicit TypeScript-only final controller decision. `DispositionService.record` validates one existing Candidate and one existing Review belonging to it through separate read-only stores, then `SqliteDispositionStore` atomically persists one immutable schemaVersion 1, status-free record plus one `disposition.recorded` event. Its fixed-length SHA-256 Candidate-derived ID allows exactly one disposition per Candidate; exact semantic replay returns the original without a second event, while a different Review ID, decision, `decidedBy`, or rationale raises an explicit conflict, including concurrent attempts from independent stores. The record stores only `accept` or `discard`, bounded non-empty `decidedBy` and rationale, and `createdAt`; its decidedBy/rationale fields use fixed UTF-8 bounds and the durable projection uses the existing 16 MiB record budget. It never applies, promotes, commits, merges, pushes, deletes, or otherwise mutates the Candidate artifact or canonical workspace. Job success remains technical execution evidence and is not acceptance; there is no CLI/HTTP workflow or automatic disposition.
...
Use `agentknot task` for the minimal WorkOrder-rooted developer loop, `task-candidate` and `task-review` for its explicit evidence steps, `agentknot run` for an already bounded leaf Job without a WorkOrder, or `agentknot orchestrate` with a controller-authored assessment when deterministic policy should decide whether and how to dispatch it.
...
| **Current** | Minimal immutable, schemaVersion 1, status-free `DispositionRecord` final controller decision linked to one Candidate and one belonging Review. | `DispositionService.record` validates both source records read-only and `SqliteDispositionStore` atomically persists one fixed-length Candidate-derived identity plus `disposition.recorded`. Accept/discard, exact replay, different-input conflict, concurrent conflict, restart, defensive-copy, and source-nonmutation behavior are covered; the record never applies or promotes an artifact, changes a lifecycle status, or treats Job success as acceptance, and no Disposition CLI/HTTP workflow exists. |
...
The minimal Disposition surface is TypeScript-only: `DispositionService.record` accepts one existing Candidate ID, one existing belonging Review ID, `accept` or `discard`, and bounded non-empty `decidedBy` and rationale; `DispositionService` and `SqliteDispositionStore` expose record/create, get, getForCandidate, list, and event-cursor operations. Creation validates the two source records read-only, uses one deterministic fixed-length Candidate-derived identity, atomically persists one `disposition.recorded` event, and rejects general save or unsupported schema versions. Exact replay is idempotent and every different semantic input conflicts, including concurrent races. It does not consume orchestration advisory verdicts, change source records or statuses, apply/promote/mutate artifacts or the canonical workspace, or add CLI/HTTP, automatic disposition, commit, merge, or push behavior.

...
export function buildPlannerPrompt(request: OrchestrationRequest, config: DelegationConfig): string {
  return [
    ...
    'Keep genuinely trivial read-only inspection or direct-answer work with no repository file deliverable upstream when direct execution and review are cheaper than planner/worker handoff; if so, state that overhead rationale rather than citing the lack of a parallel split.',
    ...
    'Use an empty subtasks array only when the work must remain upstream, cannot be bounded for a worker, or is genuinely trivial read-only inspection or direct-answer work with no repository file deliverable; never use it merely because the work is small or cannot be split. Do not wrap the assessment JSON in commentary.',
    '',
    'Task:',
    request.prompt,
  ].join('\n');

...
export function buildPlannerPrompt(request: OrchestrationRequest, config: DelegationConfig): string {
  return [
    ...
    'Keep objectively trivial work upstream when direct execution and review are cheaper than planner/worker handoff; if so, state that overhead rationale rather than citing the lack of a parallel split.',
    ...
    'Use an empty subtasks array only when the work must remain upstream, cannot be bounded for a worker, or is objectively cheaper to execute and review directly; never use it merely because the work cannot be split. Do not wrap the JSON in commentary.',
    '',
    'Task:',
    request.prompt,
  ].join('\n');

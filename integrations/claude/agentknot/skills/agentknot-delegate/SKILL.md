...
3. Admission is durable but may be queued. Call `agentknot_orchestration_wait` with the exact `id` and last `afterSequence`, omitting `waitMs`. A terminal result contains the handoff; an active result returns the same `id` and `nextSequence`, which must be reattached without resubmission. Reserve `agentknot_orchestration_follow` for one-batch diagnostics and `agentknot_orchestration_status` for an immediate snapshot. Do not repeatedly list or dump jobs.
